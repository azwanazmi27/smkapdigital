...
## 2026-09-23 Gemini MALFORMED repair

The owner observed Gemini `MALFORMED` after a live request. The old Gemini 3 JSON request capped output at 1,200 tokens; Google's documentation says this cap includes thinking tokens and can produce an empty response at `MAX_TOKENS`. This is a plausible cause, not a captured raw finish reason. For Gemini 3 JSON only, the request now uses at least 4,096 output tokens and `thinkingLevel=low`; Gemini 2.5 and text requests retain their previous settings. Local build and 106 application tests passed. Direct Worker deployment `0fac1dc5-2646-4e50-b0c2-1d7954c3ad04` retained all 13 bindings/secrets. An authenticated synthetic OPR request returned 200, populated all three fields, and live Worker tail recorded `gemini-3.5-flash` started then **succeeded** in 4,187 ms with no fallback. The unsaved draft was closed. The first post-deploy browser attempt had a transient `Failed to fetch`; the subsequent two synthetic attempts succeeded. Source: https://ai.google.dev/gemini-api/docs/generate-content/thinking .
