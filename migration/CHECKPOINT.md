...
## 2026-09-23 — e-Tempahan production repair and synthetic check
- Authenticated production status initially failed intermittently. Cloudflare observability recorded `E-Tempahan list error POST required` and a non-JSON HTML response from the Google Apps Script redirect. Single-day reads sometimes succeeded, while a 2-day and default 7-day `Promise.all` read failed. A Worker-only sequential retry patch still made the week too slow and failed; it is superseded by the range endpoint below.
- Added `etempahan_list_range` to the existing school Apps Script project and updated the **same** production web-app deployment ID/URL to version 7. It reads `REKOD_TEMPAHAN` once for an inclusive range; no trigger, scope, sharing, token or Sheet location was changed. Worker `/api/etempahan?from=...&to=...` now calls this action once, with a sequential legacy fallback for older bridge versions. Read-only Apps Script calls retry transient failures; create/delete calls are never retried to avoid duplicate writes.
- With the owner's approval, created a dummy `Bilik Media` booking for 2026-09-24 23:50–23:55 under `Ujian Sistem SMKAP`, using the school test email. The portal returned reference `ET-20260923-103642-E3174E` and reported confirmation email sent (inbox receipt not independently checked). A one-day read showed the row as approved. Canceled it through the portal; subsequent 7-day read showed `Dibatalkan`, the delete action was hidden, and the 2026-09-24 room status showed `KOSONG`. The canceled Sheet row is retained for audit; no real booking was modified.
- Corrected the default booking dates to Malaysia local time. Final Worker version `b7be2e58-62c7-4c3f-a958-12fb9f63bb0d`; previous rollback version `4bda3401-78d5-42a1-877f-9129e69975f5`. Build PASS, application tests 105/105 PASS. Cloudflare settings after deployment retained `AI_PROVIDER_ORDER=gemini,groq,openrouter,cloudflare`, `GEMINI_MODEL=gemini-3.5-flash`, encrypted AI keys and Apps Script URL/token. Production 7-day list and canceled room status PASS. A separate Google inbox check and tests under non-admin roles remain NOT TESTED.
- The production `Muat turun PDF` action generated `Senarai-Penggunaan-Bilik-2026-09-23-hingga-2026-09-29.pdf`; the downloaded artifact was identified locally as a one-page PDF 1.3. Its visual/content accuracy was not independently reviewed.

## 2026-09-23 — Gemini response repair
- Owner reports production login and OPR create/delete were successful; those user-run checks were not independently replayed here.
- Owner observed Gemini `MALFORMED`. For Gemini 3 JSON responses, increased the output cap to 4,096 and set low thinking level. This addresses a documented possible empty-output case when thinking consumes the output cap; the original response finish reason was not captured.
- Build and 106 application tests PASS. Direct production Worker version `0fac1dc5-2646-4e50-b0c2-1d7954c3ad04` deployed with `--keep-vars`; all 13 prior bindings and secret names remained present. Live synthetic OPR generation returned 200 and populated all three fields. Worker tail identified Gemini itself as the successful provider in 4,187 ms. Draft exited without saving. A separate initial browser request showed transient `Failed to fetch`, so network reliability is not fully proven.

## 2026-09-23 — E-Tempahan status response and cache
- Single-day status reads previously waited for an uncached Google Sheet call with read-only retries, sometimes longer than the visible loading state should last. Added a per-date Cloudflare Cache API entry with a 20-second TTL, read after authentication and with per-user permissions derived after retrieval. Successful create/cancel invalidates affected dates locally; other Cloudflare data centers can retain a prior snapshot for up to 20 seconds. The Google Sheet bridge still checks conflicts when saving a booking.
- The E-Tempahan status panel now stops the spinner after 4.5 seconds and says the result is unverified, with a retry button. A slow read is allowed to complete in the background so it can populate the cache and update the panel when it finishes. The panel displays its check time and whether the result came from cache or Google Sheet; it never presents an unverified empty room as available.
- Focused cache/deadline tests 2/2 PASS, application suite 108/108 PASS, production build PASS. Deployed Worker `portal` version `1ab96b4f-a92b-4fd4-8ab0-97b9e8fe490d` with existing variables retained; previous rollback version `a1792011-ec15-4002-982b-6d8b248b31b1`. Authenticated browser status showed `Cache` and a check time, room cards rendered, and no current console errors. Cold Google Sheet reads can still exceed five seconds; the visible panel has a deadline rather than a guarantee that the Sheet itself answers within that time.

## 2026-09-23 — Relief coordinator access PIN
- The production Worker had no `RELIEF_ACCESS_PIN` secret, so the coordinator PIN could not pass verification. Stored the user-specified six-digit value as a Cloudflare Worker secret; the value is not recorded in this repository. Production `POST /api/relief-legacy/access/verify` returned 204 for the specified PIN and 401 for a wrong PIN. No Relief data was changed.
- `RELIEF_ADMIN_PIN` and `RELIEF_DELETE_PASSWORD` remain separate and are not configured as Worker secrets. The coordinator PIN must not be treated as the admin PIN or delete password. Further work on those controls requires separate configuration and authorization.

## 2026-09-23 — Shared E-Keberadaan and E-Relief PIN (supersedes the separate-admin-PIN note above)
- The user requested the same six-digit PIN for E-Keberadaan admin and E-Relief, with a change-PIN control in the admin screen. Both verification routes, E-Keberadaan reason management, Relief coordinator/teacher management, and teacher review now use one shared verifier. Initial value comes from the existing `RELIEF_ACCESS_PIN` Worker secret; the value is not stored in Git.
- Added an admin-only change endpoint requiring a live portal admin session, current PIN and a different six-digit new PIN. It stores only a salted PBKDF2 hash in D1. After a change, the D1 hash supersedes the initial Worker secret for both modules. The separate absence-delete password is unchanged and not configured.
- Added the change form to the E-Keberadaan Admin screen. Browser verified the existing PIN opens Admin and the `PIN semasa`, `PIN baharu`, `Ulang PIN baharu` fields and `Tukar PIN` button render. No live PIN change was submitted because no new PIN was requested. Live read-only verification: correct PIN returned 204 on both Relief access/admin verification; wrong PIN returned 401. E-Keberadaan invalid-reason probe returned 400 with the correct PIN versus 403 with a wrong PIN, without changing data; teacher-review GET returned 200 versus 401. Application tests 110/110 and build passed. Production Worker version `3de4ef2b-626a-460f-a21d-10aec23fc5cc`; rollback reference `1ab96b4f-a92b-4fd4-8ab0-97b9e8fe490d`.

## 2026-09-23 — AI fill for one certificate
- The admin's ordinary certificate upload now offers `Baca satu sijil dengan AI` when exactly one PDF/JPG/PNG is selected. An image is converted to PDF before the existing admin-only OCR endpoint processes it. The response proposes program title and recipient, date, venue, level, field, unit, and achievement; it never saves automatically and leaves uploader identity unchanged.
- The OCR endpoint validates the new field category and limits input/response sizes. Application tests 112/112, build and `git diff --check` passed. Deployed Worker version `adee9d21-5483-4a20-9cf2-5c887636f00c`; rollback version `3de4ef2b-626a-460f-a21d-10aec23fc5cc`.
- Authenticated production browser test used a synthetic PDF only. The UI enabled the AI button for one file, returned a completed suggestion, and filled program/recipient, venue, Negeri, Inovasi, unit and achievement as printed. No record was saved and no real certificate was transmitted in this test. The user must review AI fields before saving; real-certificate accuracy is not established by this synthetic test.

## 2026-09-23 — E-Kunjung intermittent Google response
- Production `GET /api/ekunjung` was intermittent: one cold attempt returned 502 after 16.4 seconds, subsequent calls returned 200 in 6.1 and 0.45 seconds. The bridge previously parsed every Google response as JSON, so an HTML/timeout response surfaced as `Unexpected token '<'`. No evidence of a failed Sheet write was found from the read test.
- The Worker now checks the Apps Script content type, bounds read attempts and mutation waits, retries only read-only active-list requests, and returns an explicit uncertain-outcome message for a failed mutation. It never automatically retries create/checkout, avoiding duplicate writes. The browser catches non-JSON Worker responses with a Malay explanation. Active-list reads use a 15-second fresh edge cache with at most 60 seconds of marked stale fallback; successful create/checkout invalidates the local cache. Other data centers may retain a prior active list for up to 60 seconds, but the Sheet remains authoritative for checkout.
- Build and 112 application tests passed. Deployed Worker `portal` version `9de29cfd-107c-4faf-aaf9-f9b6f162a2f4` (previous version `c596087e-4c1d-4ada-b799-23ad0a3615bc`). A synthetic visitor `EK-20260923-123913-8E71AD` was created in 4.3 seconds and checked out in 2.3 seconds; a subsequent active-list GET returned 200 and confirmed it was absent. The test row remains in the school Sheet and its test photo in the Drive root for audit; no real visitor record was modified. Cold Google reads can still fail and are not claimed fully resolved.

## 2026-09-23 — Optional E-Kunjung photo and reliable admin PDF
- Updated the same production Apps Script web-app deployment to version 8, retaining its URL, execution identity and access setting. `createEkunjung_` now accepts no photo, leaves the image-URL Sheet cell empty and skips Drive file creation; photo uploads still use the existing folder. Added a read-only `ekunjung_by_date` action for a complete date export. The editor source was copied back and matched the intended change byte-for-byte after saving.
- The Worker accepts an absent photo while retaining validation for supplied JPEG/PNG. The form labels the photo optional, lets the visitor remove a selected photo, and reviews/saves without one. A synthetic no-photo check-in `EK-20260923-125645-D55EC7` returned 200 in 6.8 seconds. Its initial check-out timed out at 20 seconds with an uncertain response, but a fresh active-list read showed it absent: Google had completed the write. Added read-only Sheet reconciliation for ambiguous check-out, then the same request returned the recorded `12:57` exit and updated the D1 archive. No duplicate check-out was created.
- Admin PDF is now generated server-side, so it does not depend on a stale browser `jsPDF` chunk. It reads full rows from Google Sheet, merges portal archive rows by ID, and falls back to a visibly marked portal-only PDF if Google is unavailable. No-data plus unavailable Google returns 503 rather than a misleading empty result. Admin-only access was verified: unauthenticated PDF request returned 403. Authenticated browser downloaded a valid two-page PDF for 2026-09-23; its footer confirmed Google Sheet and portal records, and showed 9 rows including the no-photo test row and its exit time. A prior fallback PDF was valid but contained only 4 portal rows and was marked incomplete. No real visitor row was changed by testing.
- Build and 112 application tests passed. Final Worker version `3e52e11a-b5d2-4193-aeef-15a4ce506113`; preceding rollback version `8d83d139-1657-46e0-a1c1-06185accdb39`. The synthetic Sheet row remains for audit. Apps Script latency remains intermittent; no guarantee of sub-five-second Google writes is claimed.
