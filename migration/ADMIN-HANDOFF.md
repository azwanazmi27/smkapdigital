...
| R2 put/get | Remote CLI bytes/checksum PASS | Browser upload needs extension local-file permission; NOT an application permission defect |
...
| Browser local-only state migration | NOT TESTED across users | User-device inventory/transfer process; server backup is insufficient |
...
No claim that every non-admin task is complete: browser file upload, full per-role E2E, UI/network/log inspection and browser-local state checks still need finishing. Do not classify those as completed or solely administrator-blocked.
