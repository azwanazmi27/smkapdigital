...
coordinatorStyle.textContent = `
  .smk-coordinator-launch{position:fixed;right:18px;bottom:18px;z-index:30;min-height:46px;padding:10px 14px;border:1px solid #72d9cf;border-radius:12px;color:#052f39;background:#d8fff8;box-shadow:0 10px 28px rgba(3,38,48,.26);font:800 13px/1.1 Arial,sans-serif;cursor:pointer}
  .smk-coordinator-modal{position:fixed;z-index:80;inset:0;display:grid;place-items:center;padding:18px;background:rgba(3,27,36,.68)}.smk-coordinator-card{width:min(580px,100%);max-height:min(720px,calc(100vh - 36px));overflow:auto;padding:22px;border-radius:18px;color:#173b43;background:#f7fffd;box-shadow:0 24px 56px rgba(0,0,0,.35)}
  .smk-coordinator-card header{display:flex;gap:14px;align-items:flex-start;justify-content:space-between}.smk-coordinator-card h2{margin:0;color:#093e4c;font-size:22px}.smk-coordinator-card p{margin:7px 0 18px;line-height:1.45}.smk-coordinator-close{border:0;color:#164753;background:transparent;font-size:28px;line-height:1;cursor:pointer}.smk-coordinator-list{display:grid;gap:9px}.smk-coordinator-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px}.smk-coordinator-row input,.smk-coordinator-add input{min-width:0;min-height:44px;padding:9px 11px;border:1px solid #9bbfbc;border-radius:9px;color:#173b43;background:#fff;font:inherit}.smk-coordinator-row button,.smk-coordinator-add button,.smk-coordinator-save{min-height:44px;padding:9px 13px;border:1px solid #31776f;border-radius:9px;color:#fff;background:#176b61;font:800 14px Arial;cursor:pointer}.smk-coordinator-row button{border-color:#ae5660;background:#a6424e}.smk-coordinator-add{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;margin-top:13px}.smk-coordinator-save{width:100%;margin-top:16px;background:#0f6070}.smk-coordinator-message{min-height:20px;margin-top:12px!important;color:#a03542;font-weight:700}.smk-coordinator-message.ok{color:#176b61}@media(max-width:640px){.smk-coordinator-launch{right:12px;bottom:12px}.smk-coordinator-card{padding:18px}.smk-coordinator-add{grid-template-columns:1fr}.smk-coordinator-add button{width:100%}}
`;
...
const openCoordinatorSettings = async () => {
  const overlay = document.createElement("div"); overlay.className = "smk-coordinator-modal";
  overlay.innerHTML = `<section class="smk-coordinator-card" role="dialog" aria-modal="true" aria-labelledby="smk-coordinator-title"><header><div><h2 id="smk-coordinator-title">Tetapan Penyelaras Relief</h2><p>Tambah, ubah atau buang nama penyelaras yang boleh dipilih semasa menjana jadual relief.</p></div><button type="button" class="smk-coordinator-close" aria-label="Tutup">×</button></header><div class="smk-coordinator-list"></div><div class="smk-coordinator-add"><input aria-label="Nama penyelaras baharu" maxlength="120" placeholder="Nama penyelaras baharu"><button type="button">Tambah</button></div><p class="smk-coordinator-message" aria-live="polite"></p><button type="button" class="smk-coordinator-save">Simpan perubahan</button></section>`;
  const close = () => overlay.remove(); overlay.addEventListener("mousedown", event => {if(event.target === overlay) close();}); overlay.querySelector(".smk-coordinator-close").onclick = close;
  const list = overlay.querySelector(".smk-coordinator-list"), message = overlay.querySelector(".smk-coordinator-message"), input = overlay.querySelector(".smk-coordinator-add input"), add = overlay.querySelector(".smk-coordinator-add button"), save = overlay.querySelector(".smk-coordinator-save");
