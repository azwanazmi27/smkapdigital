...
function defaultMappings(input:RegisterInput){
  ...
  const epanitia=source.includes("opr")||type.includes("opr")||type.includes("kertas kerja")?"05":type.includes("minit")?"03":type.includes("pentaksiran")?"04":type.includes("carta organisasi")?"01":"08";
...
export async function addDocumentVersion(actor:Actor,documentId:string,file:DriveFile,status="draft"){
  const doc=await env.DB.prepare("SELECT id FROM documents WHERE id=? AND archived_at=''").bind(documentId).first();if(!doc)throw new Error("Dokumen induk tidak ditemui");
  const duplicate=await env.DB.prepare("SELECT id FROM document_versions WHERE drive_file_id=?").bind(file.id).first<{id:string}>();if(duplicate)return {versionId:duplicate.id,created:false};
  ...
  await env.DB.batch([
    ...
    env.DB.prepare("UPDATE documents SET updated_at=? WHERE id=?").bind(createdAt,documentId),
  ]);
