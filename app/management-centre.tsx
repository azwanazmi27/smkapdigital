...
import {FolderOpen,ArrowLeft,ExternalLink,Plus,Search,FileText,MoreHorizontal,Trash2,ChevronDown} from 'lucide-react';
...
import {OprManagementLinks,type Linked} from './opr-management-links';
...
export function ManagementCentre({user,notify,initialFolder='',openSkas}:{user:{email:string;name:string;role:string}|null;notify:(s:string)=>void;initialFolder?:string;openSkas:()=>void}){
 const [year,setYear]=useState(new Date().getFullYear()),[years,setYears]=useState<Array<{year:number;status:string}>>([]),[docs,setDocs]=useState<Material[]>([]),[folder,setFolder]=useState(initialFolder),[kind,setKind]=useState(''),[query,setQuery]=useState(''),[busy,setBusy]=useState(false),[loading,setLoading]=useState(true),[error,setError]=useState(''),[adding,setAdding]=useState(false),[uploadsOpen,setUploadsOpen]=useState(false),[mode,setMode]=useState('link'),[file,setFile]=useState<File|null>(null),[title,setTitle]=useState(''),[url,setUrl]=useState(''),[notes,setNotes]=useState(''),[visibility,setVisibility]=useState('private'),[docType,setDocType]=useState(managementDocumentTypes[0]),[requestId,setRequestId]=useState(''),[mapping,setMapping]=useState<Material|null>(null),[standard,setStandard]=useState('2'),[unit,setUnit]=useState('');
 ...
 const folderCounts=(folderId:string)=>{const uploads=docs.filter(d=>inManagementFolder(d.folderId,folderId)).length,reports=linked.filter(d=>inManagementFolder(d.folderId,folderId)).length;return {uploads,reports,total:uploads+reports};};
 ...
 async function save(e:React.FormEvent){
  ...
  try{
   ...
   setAdding(false);setKind('');setQuery('');setTitle('');setUrl('');setFile(null);setNotes('');notify('Bahan berjaya disimpan.');await load(year);
 ...
 async function map(e:React.FormEvent){e.preventDefault();if(mapping)await saveMapping(mapping,standard,unit);}
 async function saveMapping(target:Material,standardCode:string,unitName:string){
  if(busy)return;setBusy(true);setError('');
  try{const r=await fetch('/api/pengurusan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'map',id:target.id,standardCode,unitName}),signal:AbortSignal.timeout(20000)}),data=await r.json();if(!r.ok)throw Error(data.error);setMapping(null);notify('Pemetaan disimpan untuk perakuan di Pusat SK@S.');await load(year);}catch(e){setError(e instanceof Error?e.message:'Pemetaan gagal.');}finally{setBusy(false);}
 }
 ...
 <nav className="management-navigation" aria-label="Folder pengurusan"><button disabled={busy} onClick={()=>navigate('')}>Semua bahagian</button>{folder&&<><span className="management-current-path" aria-current="location">{managementPath(folder)}</span><button disabled={busy} onClick={()=>navigate(node?.parent||'')}><ArrowLeft/> Kembali satu tahap</button></>}</nav>
 ...
 {!folder?<div className="management-folders">{managementSections.map(s=>{const count=folderCounts(s.id);return <button key={s.id} onClick={()=>navigate(s.id)}><FolderOpen/><strong>{s.name}</strong><span><b>{count.total} bahan</b><small>{count.uploads} muat naik · {count.reports} laporan</small></span></button>})}</div>:<>
 ...
 {children.length>0&&<div className="management-folders">{children.map(f=>{const count=folderCounts(f.id);return <button key={f.id} onClick={()=>navigate(f.id)}><FolderOpen/><strong>{f.name}</strong><span><b>{count.total} bahan</b><small>{count.uploads} muat naik · {count.reports} laporan</small></span></button>})}</div>}
 {node&&<><div className="management-toolbar"><button disabled={busy||!yearOpen} onClick={()=>{setAdding(true);setMapping(null);setRequestId(crypto.randomUUID());setDocType(kind||managementDocumentTypes[0]);}}><Plus/> Tambah fail / pautan</button>{admin&&<button onClick={openSkas}>Buka Pusat SK@S</button>}</div>{!yearOpen&&<p>Tahun ditutup atau belum dibuka. Dokumen masih boleh dibaca.</p>}<div className="management-types">{managementDocumentTypes.map(t=><button key={t} aria-pressed={kind===t} onClick={()=>setKind(kind===t?'':t)}>{t}<b>{docs.filter(d=>inManagementFolder(d.folderId,folder)&&d.documentType===t).length+linked.filter(d=>inManagementFolder(d.folderId,folder)&&d.documentType===t).length}</b></button>)}</div></>}
 </>}
 ...
 {mapping&&<form className="management-form" onSubmit={map}><h3>Petakan bahan ke SK@S</h3><p>{mapping.title}</p><p>Cadangan awal berdasarkan folder, jenis dokumen dan tajuk, bukan analisis kandungan fail. Semak dokumen sebelum memilih standard.</p><fieldset disabled={busy}><label>Standard<select value={standard} onChange={e=>setStandard(e.target.value)}>{skasStandards.map(([c,n])=><option key={c} value={c}>{c} · {n}</option>)}</select></label><label>Unit / pecahan evidens<input value={unit} maxLength={120} onChange={e=>setUnit(e.target.value)} required/></label><div className="management-toolbar"><button type="submit">Simpan pemetaan untuk semakan</button><button type="button" onClick={()=>setMapping(null)}>Batal</button></div></fieldset>{busy&&<p role="status">Sedang menyimpan pemetaan…</p>}</form>}
 <label className="management-search"><span><Search aria-hidden="true"/><b>Cari tajuk, unit atau nama</b></span><input type="search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Taip untuk mencari"/></label>
 <OprManagementLinks year={year} folder={folder} kind={kind} query={query} admin={admin} yearOpen={yearOpen} onReports={setLinked}/>
 <section className="management-group"><button className="management-group-toggle" aria-expanded={uploadsOpen} onClick={()=>setUploadsOpen(value=>!value)}><span><strong>Fail / pautan dimuat naik</strong><small>{selectedDocs.length} bahan dalam pilihan ini</small></span><ChevronDown aria-hidden="true"/></button>
 {uploadsOpen&&(!selectedDocs.length?<p className="management-empty">Tiada muat naik manual dalam pilihan ini.</p>:<div className="management-materials">{selectedDocs.map(doc=><article className="management-upload-card" key={doc.id}><FileText/><div><h4>{doc.title}</h4><p>{managementPath(doc.folderId)}</p><p>{doc.documentType} · {doc.ownerName} · {new Date(doc.createdAt).toLocaleDateString('ms-MY')}</p><p>{doc.visibility==='private'?'Akses terhad':'Warga sekolah'}</p>{doc.notes&&<p>{doc.notes}</p>}{admin&&!doc.mappingStatus&&<ManagementSuggestionCard doc={doc} disabled={busy||!yearOpen} onUse={s=>void saveMapping(doc,s.standardCode,s.unitName)}/>}<div className="management-material-actions"><div className="management-open-actions"><a href={doc.openUrl} target="_blank" rel="noopener noreferrer">Buka dokumen <ExternalLink/></a>{doc.uploaded&&<a href={doc.sourceUrl} target="_blank" rel="noopener noreferrer">Google Drive</a>}</div>{admin&&(doc.mappingStatus?<span className="management-status">{doc.mappingStatus==='approved'?'Diperakui':doc.mappingStatus==='pending'?'Menunggu perakuan':'Perlu semakan'} · Standard {doc.standardCode}</span>:<button disabled={busy||!yearOpen} onClick={()=>{setAdding(false);setMapping(doc);const s=suggestManagementMaterial(doc);setStandard(s?.standardCode||'2');setUnit(s?.unitName||managementPath(doc.folderId).slice(0,120));}}>Pemetaan manual</button>)}{doc.canDelete&&<details className="management-more"><summary aria-label={`Tindakan lain untuk ${doc.title}`}><MoreHorizontal aria-hidden="true"/></summary><div><button className="management-delete" disabled={busy||(!admin&&!yearOpen)} onClick={()=>void remove(doc)}><Trash2 aria-hidden="true"/>{busy?'Sedang memproses…':'Padam bahan'}</button></div></details>}</div></div></article>)}</div>)}</section>
 </>}
 </div>;
}
