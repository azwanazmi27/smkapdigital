...
export function ManagementCentre({user,notify,initialFolder='',openSkas}:{user:{email:string;name:string;role:string}|null;notify:(s:string)=>void;initialFolder?:string;openSkas:()=>void}){
 ...
 {!folder?<div className="management-folders">{managementSections.map(s=><button key={s.id} onClick={()=>navigate(s.id)}><FolderOpen/><strong>{s.name}</strong><span>{docs.filter(d=>inManagementFolder(d.folderId,s.id)).length+linked.filter(d=>inManagementFolder(d.folderId,s.id)).length} bahan boleh diakses</span></button>)}</div>:<>
 ...
 {children.length>0&&<div className="management-folders">{children.map(f=><button key={f.id} onClick={()=>navigate(f.id)}><FolderOpen/><strong>{f.name}</strong><span>{docs.filter(d=>inManagementFolder(d.folderId,f.id)).length+linked.filter(d=>inManagementFolder(d.folderId,f.id)).length} bahan boleh diakses</span></button>)}</div>}
