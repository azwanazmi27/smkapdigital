...
export function displayData(data:Data,users:Staff[]):Data{const name=(id:string)=>users.find(u=>u.id===id)?.name||'[Belum dipilih]';const visit=(v:any,k=''):any=>personKeys.has(k)&&typeof v==='string'?name(v):peopleArrays.has(k)&&Array.isArray(v)?v.map(name):Array.isArray(v)?v.map(x=>visit(x)):v&&typeof v==='object'?Object.fromEntries(Object.entries(v).map(([key,x])=>[key,visit(x,key)])):v;return {...visit(data),panelId:panelName(data.panelId)};}
