...
type Linked={id:string;title:string;category:string;folderId:string;openUrl:string;mappingStatus:string;standardCode:string};
export function OprManagementLinks({year,folder,admin,openSkas}:{year:number;folder:string;admin:boolean;openSkas:()=>void}){
 ...
 return <section className="management-materials"><h3>OPR berkaitan ({shown.length})</h3><p>Rujukan kepada laporan asal. Tidak perlu muat naik semula. Pemetaan SK@S masih memerlukan perakuan.</p><button onClick={()=>setRefresh(n=>n+1)} disabled={loading}>Segarkan rujukan OPR</button>
 {loading?<p role="status">Membaca rujukan OPR…</p>:error?<p role="alert">{error}</p>:shown.length?shown.map(r=><article key={r.id}><div><h4>{r.title}</h4><p>{r.folderId?managementPath(r.folderId):'Perlu semakan: kategori belum mempunyai padanan folder yang jelas.'}</p><p>{r.category}</p><p>{r.mappingStatus==='approved'?'Evidens diperakui':r.mappingStatus?'Pemetaan SK@S: perlu semakan / perakuan':'Calon evidens SK@S'}{r.standardCode?' · Standard '+r.standardCode:''}</p><div className="management-toolbar">{r.openUrl&&<a href={r.openUrl} target="_blank" rel="noopener noreferrer">Buka OPR asal</a>}{admin&&<button onClick={openSkas}>Semak pemetaan di SK@S</button>}</div></div></article>):<p>Tiada OPR yang mempunyai padanan bagi folder dan tahun ini.</p>}
 </section>;
}
