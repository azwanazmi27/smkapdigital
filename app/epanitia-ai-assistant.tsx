...
export function PanitiaAIAssistant({template,panel,data,disabled,onApply}:{template:string;panel:string;data:Record<string,unknown>;disabled:boolean;onApply:(patch:PanitiaAIPatch)=>void}){
 ...
 return <section className="evidence-ai"><div className="evidence-ai-heading"><Sparkles aria-hidden="true"/><strong>Sediakan dokumen dengan Bantuan Bang Wan</strong></div><AIUsageNote/><p>Terangkan isi yang diperlukan. Semak draf Bang Wan sebelum memasukkannya ke dalam borang.</p><label>Catatan untuk Bang Wan<textarea rows={3} maxLength={6000} value={notes} disabled={busy||disabled} onChange={e=>setNotes(e.target.value)} placeholder="Contoh: Sediakan bidang tugas setiausaha panitia untuk mengurus mesyuarat, minit dan dokumentasi."/></label><button type="button" onClick={()=>void generate()} disabled={busy||disabled||notes.trim().length<10}><Sparkles aria-hidden="true"/>{busy?'Sedang menyediakan draf…':'Jana draf dengan Bantuan Bang Wan'}</button>{error&&<p role="alert">{error}</p>}{patch&&<div className="evidence-ai-result"><strong>Draf untuk semakan</strong>{Object.entries(patch).map(([field,value])=><div key={field}><b>{panitiaAIFields[template][field]?.label}</b>{typeof value==='string'?<p>{value}</p>:<ol>{value.map((row,i)=><li key={i}>{typeof row==='string'?row:Object.entries(row).filter(([,v])=>v).map(([column,text])=><p key={column}><b>{panitiaAIFields[template][field]?.columns?.[column]}: </b>{text}</p>)}</li>)}</ol>}</div>)}<p>Nama, tarikh, angka, harga dan kelulusan perlu disemak sendiri. Kandungan teks berkaitan dalam borang akan diganti apabila draf ini digunakan.</p><button type="button" disabled={disabled} onClick={()=>{onApply(patch);setResult(null);}}>Gunakan draf dalam borang</button></div>}</section>;
}
