  ...
  return { name: "gemini" as const, model, configured: Boolean(apiKey), async generate(input: AIGenerateInput, signal?: AbortSignal): Promise<AIGenerateResult> {
    ...
    try {
      response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`, {
        ...
        ] }], generationConfig: { temperature: input.temperature ?? 0.2, maxOutputTokens: input.maxTokens ?? 1200, ...(input.responseFormat === "json" ? { responseMimeType: "application/json" } : {}) } }), signal,
      });
