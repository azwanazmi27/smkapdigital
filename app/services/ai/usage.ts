...
export function usagePolicy(actor:Actor,config:NodeJS.ProcessEnv=process.env){
  const value=Number(config.AI_DAILY_REQUEST_LIMIT||3);
  const limit=Number.isInteger(value)&&value>0?value:3;
  const exempt=(config.AI_DAILY_EXEMPT_ROLES??'admin,super_admin').split(',').map(s=>s.trim()).includes(actor.role);
  return {limit,exempt};
}
...
export async function reserveAIUsage(actor:Actor):Promise<{usage:AIUsage;error?:string}>{
  ...
  const row=await env.DB.prepare(`INSERT INTO ai_daily_usage(id,user_id,day,requests,minute,minute_requests,updated_at)
    ...
    updated_at=excluded.updated_at WHERE (?=1 OR ai_daily_usage.requests<?) AND (ai_daily_usage.minute!=excluded.minute OR ai_daily_usage.minute_requests<4)
    RETURNING requests`).bind(`${userId}:${day}`,userId,day,minute,now.toISOString(),p.exempt?1:0,p.limit).first<{requests:number}>();
