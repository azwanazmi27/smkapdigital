...
export async function POST(request: Request) {
  try {
    ...
    const result = await generateAI({ systemPrompt: "Anda ialah pembantu pembacaan dokumen sekolah Malaysia. Gunakan Bahasa Melayu Malaysia dan salin fakta yang benar-benar kelihatan sahaja. Jangan mereka maklumat atau mendedahkan nombor kad pengenalan.", userPrompt: "Baca halaman sijil ini. Pulangkan JSON sahaja dengan recipient, title, date (YYYY-MM-DD), venue, level (Sekolah, Daerah, Negeri, Kebangsaan, Antarabangsa atau kosong), achievement, dan confidence (0-100). Gunakan rentetan kosong jika tidak pasti. Jangan gunakan markdown.", temperature: 0, maxTokens: 700, responseFormat: "json", attachments: [{ mimeType: "application/pdf", base64 }] });
