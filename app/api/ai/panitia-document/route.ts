...
export async function POST(request:Request){
 try{
  const actor=await portalActor(request);if(!actor)return Response.json({error:'Sila log masuk untuk menggunakan Bantuan Bang Wan.'},{status:401});
  ...
  if(!fields||notes.length<10)return Response.json({error:'Pilih templat dan masukkan sekurang-kurangnya 10 aksara catatan untuk Bang Wan.'},{status:400});
