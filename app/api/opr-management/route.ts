...
export async function POST(request:Request){
 ...
 try{
  ...
  await db.prepare('INSERT INTO management_report_folders (report_id,folder_id,updated_by,updated_at) VALUES (?,?,?,?) ON CONFLICT(report_id) DO UPDATE SET folder_id=excluded.folder_id,updated_by=excluded.updated_by,updated_at=excluded.updated_at').bind(id,folderId,me.email,new Date().toISOString()).run();
...
export async function GET(request:Request){
 ...
 try{
  ...
  const reports=rows.results.filter(isPrimaryReport).flatMap(row=>{
   ...
   return [{id:row.id,title:isAchievement?achievement.title:title,category:row.category,folderId:chosen.folderId,mappingBasis:chosen.basis,sourceModule:isAchievement?'Arkib Kejayaan':isMonitoring?'e-Pemantauan':'OPR',openUrl:safeMaterialUrl(row.viewUrl)}];
  });
