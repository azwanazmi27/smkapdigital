...
export async function GET(request:Request){
 ...
 try{
  ...
  const reports=rows.results.filter(isPrimaryReport).flatMap(row=>{
   ...
   return [{id:row.id,title:isAchievement?achievement.title:title,category:row.category,folderId:folderId||'',sourceModule:isAchievement?'Arkib Kejayaan':isMonitoring?'e-Pemantauan':'OPR',openUrl:safeMaterialUrl(row.viewUrl),mappingStatus:row.mappingStatus,standardCode:row.standardCode}];
  });
