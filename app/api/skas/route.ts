...
function candidateMappings(category:string,title:string,payload:Record<string,unknown>,stored:unknown,rule?:MappingRuleRow){
  ...
  if(destination?.id==='enam-2-3')automatic.unshift({standardCode:'3.1',standardLabel:'Pengurusan Kurikulum',domain:'Kurikulum',unitName:'Tingkatan Enam · Kecemerlangan Akademik Tingkatan Enam',evidenceType:'Program, aktiviti atau OPR',reason:'Tujuan program ialah kecemerlangan akademik Tingkatan Enam; lokasi program tidak digunakan untuk pemetaan.',confidence:'tinggi'});
  if(!rule||(category===monitoringCategory&&!automatic.length))return automatic;
...
export async function POST(request:Request){
  try{
    ...
    if(action==="import_candidate"){
      ...
      if(sourceYear!==null&&sourceYear!==year)return Response.json({error:'Tahun pemetaan mesti sama dengan tahun laporan ('+sourceYear+').'}, {status:400});
      ...
      const writes=[env.DB.prepare("INSERT INTO skas_evidence(id,school_year,domain,unit_name,evidence_type,title,standard_code,source_type,source_url,storage_key,mime_type,original_name,notes,status,submitted_by_email,submitted_by_name,verified_by_email,verified_by_name,verified_at,source_module,source_record_id,created_at,updated_at) VALUES(?,?,?,?,?,?,?,'portal',?,'','','',?,'pending',?,?,'','','',?,?,?,?)").bind(id,year,domain,unitName,evidenceType,title,standard,row.viewUrl,`${manual?"Pemetaan manual":"Pemetaan automatik"}: ${reason}`,me.email,me.name,row.category===achievementCategory?'Arkib Kejayaan':row.category===monitoringCategory?'e-Pemantauan':'OPR',reportId,now,now),env.DB.prepare("UPDATE opr_intake_metadata SET review_status='imported',updated_at=? WHERE report_id=?").bind(now,reportId)];
...
export async function PATCH(request:Request){
  try{await prepare();const me=await requireAdmin(request);if(!me)return denied();const input=await request.json() as Record<string,unknown>,id=clean(input.id,80),status=clean(input.status,30),notes=clean(input.notes,1800),changes=input.changes&&typeof input.changes==='object'?input.changes as Record<string,unknown>:null;if(!id||!allowedStatus.has(status))return Response.json({error:"Tindakan tidak sah."},{status:400});const standardCode=changes?clean(changes.standardCode,20):'',domain=changes?clean(changes.domain,80):'',unitName=changes?clean(changes.unitName,120):'',title=changes?clean(changes.title,180):'',submittedByName=changes?clean(changes.submittedByName,180):'',evidenceDate=changes?clean(changes.evidenceDate,10):'';if(changes&&(!allowedStandards.has(standardCode)||!allowedDomains.has(domain)||!allowedUnit(domain,unitName)||!allowedStandardDomain(standardCode,domain)))return Response.json({error:"Standard, bidang dan unit yang dipilih tidak sepadan."},{status:400});if(changes&&(!title||!submittedByName||!/^\d{4}-\d{2}-\d{2}$/.test(evidenceDate)||!Number.isFinite(Date.parse(`${evidenceDate}T12:00:00Z`))))return Response.json({error:"Tajuk, tarikh atau nama penyedia tidak sah."},{status:400});const now=new Date().toISOString(),createdAt=changes?`${evidenceDate}T12:00:00.000Z`:'';await env.DB.prepare("UPDATE skas_evidence SET status=?,notes=CASE WHEN ?!='' THEN ? ELSE notes END,standard_code=CASE WHEN ?!='' THEN ? ELSE standard_code END,domain=CASE WHEN ?!='' THEN ? ELSE domain END,unit_name=CASE WHEN ?!='' THEN ? ELSE unit_name END,title=CASE WHEN ?!='' THEN ? ELSE title END,submitted_by_name=CASE WHEN ?!='' THEN ? ELSE submitted_by_name END,created_at=CASE WHEN ?!='' THEN ? ELSE created_at END,verified_by_email=?,verified_by_name=?,verified_at=?,updated_at=? WHERE id=? AND status!='source_deleted'").bind(status,notes,notes,standardCode,standardCode,domain,domain,unitName,unitName,title,title,submittedByName,submittedByName,createdAt,createdAt,me.email,me.name,now,now,id).run();return Response.json({success:true});}catch(error){console.error("SKAS review",error);return Response.json({error:"Status atau maklumat evidens tidak dapat dikemas kini."},{status:500});}
}

export async function DELETE(request:Request){
  try{await prepare();const me=await requireAdmin(request);if(!me)return denied();const id=clean(new URL(request.url).searchParams.get("id"),80),row=await env.DB.prepare("SELECT storage_key AS storageKey,source_module AS sourceModule,source_record_id AS sourceRecordId FROM skas_evidence WHERE id=?").bind(id).first<{storageKey:string;sourceModule:string;sourceRecordId:string}>();if(!row)return Response.json({error:"Evidens tidak ditemui."},{status:404});if(row.storageKey)await env.FILES.delete(row.storageKey);await env.DB.prepare("DELETE FROM skas_evidence WHERE id=?").bind(id).run();if(["OPR","e-Pemantauan","Arkib Kejayaan"].includes(row.sourceModule)&&row.sourceRecordId)await env.DB.prepare("UPDATE opr_intake_metadata SET review_status='pending',updated_at=? WHERE report_id=?").bind(new Date().toISOString(),row.sourceRecordId).run();return Response.json({success:true});}catch(error){console.error("SKAS delete",error);return Response.json({error:"Evidens tidak dapat dipadam."},{status:500});}
}
