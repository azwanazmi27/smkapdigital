...
export async function DELETE(request:NextRequest,c:{params:Promise<{path:string[]}>}){const p=await parts(c),root=p[0],id=p[1]||request.nextUrl.searchParams.get("id");if(root==="absences"||root==="teachers"){if(request.headers.get("x-delete-confirmation")!=="PADAM")return json({error:"Taip PADAM untuk mengesahkan tindakan ini."},400);const actor=await portalActor(request);if(!actor)return json({error:"Sila log masuk ke portal dahulu."},401);if(root==="teachers"&&!["admin","super_admin"].includes(actor.role))return json({error:"Akaun pentadbir diperlukan."},403)}if(!id)return json({error:"ID diperlukan"},400);let sheetMirrored=true;if(root==="absences"){await env.DB.prepare("DELETE FROM absences WHERE id=?").bind(id).run();sheetMirrored=await markAbsenceDeletedInSheet(id);}else if(root==="teachers")await env.DB.prepare("DELETE FROM teachers WHERE id=?").bind(id).run();else if(root==="schedules")await env.DB.prepare("DELETE FROM relief_schedules WHERE id=?").bind(id).run();else return json({error:"Laluan tidak ditemui"},404);return json({ok:true,sheetMirrored})}
