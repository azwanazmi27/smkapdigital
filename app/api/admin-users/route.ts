...
import { permittedProfileUpdate } from "../../profile-model";
...
async function prepare(){
  ...
  const seedVersion="admin_seed_2026_09_09_v1";
  const seedClaim=await env.DB.prepare("INSERT OR IGNORE INTO portal_settings(setting_key,setting_value,updated_at) VALUES(?,'running',?)").bind(seedVersion,now).run();
  if(!seedClaim.meta.changes)return;
  ...
  await env.DB.prepare("UPDATE portal_settings SET setting_value='complete',updated_at=? WHERE setting_key=?").bind(new Date().toISOString(),seedVersion).run();
