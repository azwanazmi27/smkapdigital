...
import {canReadMaterial,isManagementAdmin,managementSource,mappingDomain,safeMaterialUrl,validSchoolYear} from '../../management-model';
...
async function row(id:string){return managementStore().db.prepare('SELECT '+columns+' FROM management_materials m WHERE m.id=?').bind(id).first<Material>();}
...
export async function GET(request:Request){
 try{
  ...
  const [years,docs]=await Promise.all([
   ...
   db.prepare('SELECT '+columns+",s.status AS mappingStatus,s.standard_code AS standardCode FROM management_materials m LEFT JOIN skas_evidence s ON s.source_module=? AND s.source_record_id=m.id WHERE m.school_year=? AND (m.visibility='staff' OR m.owner_email=? OR ?=1) ORDER BY m.created_at DESC").bind(managementSource,year,me.email,isManagementAdmin(me.role)?1:0).all<Material>(),
  ]);
  return json({year,years:years.results,materials:docs.results.map(({storageKey,...doc}:Material)=>({...doc,uploaded:Boolean(storageKey),openUrl:storageKey?'/api/pengurusan?file='+encodeURIComponent(doc.id):safeMaterialUrl(doc.sourceUrl)}))});
...
export async function POST(request:Request){
 ...
 try{
  ...
  if(input.action==='map'){
   ...
   await db.prepare("INSERT INTO skas_evidence(id,school_year,domain,unit_name,evidence_type,title,standard_code,source_type,source_url,notes,status,submitted_by_email,submitted_by_name,source_module,source_record_id,created_at,updated_at) VALUES(?,?,?,?,?,?,?,'portal',?,?,'pending',?,?,?,?,?,?) ON CONFLICT DO NOTHING")
    .bind(crypto.randomUUID(),doc.schoolYear,domain,unit,doc.documentType,doc.title,standard,sourceUrl,doc.notes,doc.ownerEmail,doc.ownerName,managementSource,doc.id,now,now).run();
