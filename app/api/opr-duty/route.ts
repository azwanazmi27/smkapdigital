...
export async function POST(request: Request) {
  try {
    ...
    const reportDate=text("reportDate",10), teachers=text("teachers",500), preparedBy=text("preparedBy",120);
    ...
    const now=new Date().toISOString(), id=text("id",80)||crypto.randomUUID();
    await env.DB.prepare("INSERT INTO opr_duty_reports (id,report_date,week_number,school_year,day_name,teachers,teacher_total,teacher_present,teacher_absent,cleanliness_status,discipline_status,safety_status,health_status,canteen_status,activity_note,general_note,prepared_by,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(report_date) DO UPDATE SET week_number=excluded.week_number,school_year=excluded.school_year,day_name=excluded.day_name,teachers=excluded.teachers,teacher_total=excluded.teacher_total,teacher_present=excluded.teacher_present,teacher_absent=excluded.teacher_absent,cleanliness_status=excluded.cleanliness_status,discipline_status=excluded.discipline_status,safety_status=excluded.safety_status,health_status=excluded.health_status,canteen_status=excluded.canteen_status,activity_note=excluded.activity_note,general_note=excluded.general_note,prepared_by=excluded.prepared_by,updated_at=excluded.updated_at").bind(id,reportDate,integer("weekNumber",53),integer("schoolYear",2100),text("dayName",30),teachers,integer("teacherTotal"),integer("teacherPresent"),integer("teacherAbsent"),text("cleanlinessStatus",30),text("disciplineStatus",30),text("safetyStatus",30),text("healthStatus",30),text("canteenStatus",30),text("activityNote"),text("generalNote"),preparedBy,now,now).run();
