...
export function EvidenceAIAssistant({title,context='',notes='',panitia=false,disabled=false,onApply,applyLabel='Gunakan cadangan'}:{title:string;context?:string;notes?:string;panitia?:boolean;disabled?:boolean;onApply:(suggestion:EvidenceAISuggestion)=>void|Promise<void>;applyLabel?:string}) {
  ...
  return <div className={`evidence-ai${open?'':' is-collapsed'}`}><button type="button" className="evidence-ai-toggle" aria-expanded={open} onClick={()=>setOpen(value=>!value)}><Sparkles aria-hidden="true"/>{open?'Tutup Bantuan Bang Wan':'Bantuan Bang Wan'}</button>{open&&<><div className="evidence-ai-heading"><Sparkles aria-hidden="true"/><strong>Bantuan Bang Wan untuk eviden</strong></div><AIUsageNote/><p>Bang Wan mencadangkan pemetaan daripada tajuk dan ringkasan. Semak dokumen asal sebelum menggunakan cadangan.</p><label>Ringkasan kandungan untuk Bang Wan<textarea rows={3} maxLength={5000} value={summary} disabled={busy||disabled} onChange={e=>setSummary(e.target.value)} placeholder="Nyatakan kandungan, tujuan dan hasil yang dibuktikan. Boleh tampal petikan dokumen di sini."/></label><button type="button" disabled={busy||disabled||title.trim().length<3} onClick={()=>void suggest()}><Sparkles aria-hidden="true"/>{busy?'Sedang memproses…':'Cadangkan pemetaan dengan Bang Wan'}</button>{title.trim().length<3&&<small>Lengkapkan tajuk dahulu.</small>}{error&&<p role="alert">{error}</p>}{suggestion&&<div className="evidence-ai-result" aria-live="polite"><strong>Standard {suggestion.standardCode} · {suggestion.standardLabel}</strong><span>{suggestion.domain} › {suggestion.unitName}</span><span>{suggestion.evidenceType}</span>{panitia&&<span>Folder panitia: {evidencePanitiaCategories.find(([id])=>id===suggestion.panitiaCategory)?.[1]}</span>}<p>{suggestion.reason}</p><small>Keyakinan {suggestion.confidence} · Cadangan untuk semakan</small><button type="button" disabled={busy||disabled||applied} onClick={()=>void apply()}>{applied?'Cadangan telah digunakan':applyLabel}</button></div>}</>}</div>;
}
