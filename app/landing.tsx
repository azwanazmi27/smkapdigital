...
export function LandingPortal() {
  ...
  const loadIdentity=async(showWelcome=false)=>{const [response,announcementResponse]=await Promise.all([fetch("/api/admin-users?resource=me",{cache:"no-store"}),fetch("/api/portal-content?view=staff",{cache:"no-store"})]),data=await response.json(),announcementData=await announcementResponse.json();if(!response.ok)throw new Error(data.error||"Log masuk tidak berjaya.");setIdentity(data.me);setStaffAnnouncements(announcementResponse.ok?announcementData.announcements||[]:[]);setAuthOpen(false);if(showWelcome){setWelcome(true);setOpen("warga");}};
  ...
  useEffect(() => { void fetchOprIndex().catch(() => {});void loadIdentity(false).catch(()=>{}); }, []);
  useEffect(() => {
  ...
  const logout=()=>{void fetch("/api/session",{method:"DELETE"});window.google?.accounts.id.disableAutoSelect?.();setIdentity(null);setWelcome(false);setProfileOpen(false);setAuthOpen(false);setOpen(null);notify("Anda telah log keluar");};
...
function AdminPanel({ notify }: { notify: (message: string) => void }) {
  ...
  const [pushDraft,setPushDraft]=useState({title:"",body:"",url:"/",audience:"Semua warga"});
  ...
  const sendPush=async()=>{try{setPushSending(true);const response=await fetch("/api/push",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"send",...pushDraft})}),data=await response.json() as{error?:string;sent?:number;failed?:number};if(!response.ok)throw new Error(data.error||"Notifikasi tidak dapat dihantar");notify(`Notifikasi dihantar kepada ${data.sent||0} peranti${data.failed?` · ${data.failed} gagal`:""}`);setPushDraft({title:"",body:"",url:"/",audience:"Semua warga"});await loadPush();}catch(e){notify(e instanceof Error?e.message:"Notifikasi tidak dapat dihantar");}finally{setPushSending(false);}};
...
const exportJson=async(kind:"ekeberadaan"|"ekunjung"|"etempahan")=>{setExporting(kind);try{let payload:unknown;if(kind==="ekeberadaan"){const response=await fetch("/api/ekeberadaan?resource=all",{cache:"no-store"});payload=await response.json();if(!response.ok)throw new Error("Data E-Keberadaan tidak dapat dieksport");}else if(kind==="ekunjung"){const response=await fetch("/api/ekunjung",{cache:"no-store"});payload=await response.json();if(!response.ok)throw new Error("Data E-Kunjung tidak dapat dieksport");}else{const year=new Date().getFullYear(),all:unknown[]=[];for(let month=0;month<12;month+=1){const from=`${year}-${String(month+1).padStart(2,"0")}-01`,end=new Date(year,month+1,0),to=`${year}-${String(month+1).padStart(2,"0")}-${String(end.getDate()).padStart(2,"0")}`;const response=await fetch(`/api/etempahan?from=${from}&to=${to}`,{cache:"no-store"}),data=await response.json();if(response.ok&&Array.isArray(data.bookings))all.push(...data.bookings);}payload={exportedAt:new Date().toISOString(),year,bookings:all};}saveDownload(new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),`${kind}-SMKAP-${new Date().toISOString().slice(0,10)}.json`);notify(`Data ${kind} berjaya dimuat turun`);}catch(e){notify(e instanceof Error?e.message:"Eksport tidak berjaya");}finally{setExporting("");}};
  ...
  return <div className="admin-console"><header><div><span className="modal-overline">KAWALAN PENTADBIR</span><h2 id="folder-title">Pusat Pentadbiran Portal</h2><p>{me.name} · {me.role==="super_admin"?"Super Admin":"Admin"}</p></div><button onClick={logout}>Log keluar</button></header>
    ...
    {tab==="notifications"&&<section className="admin-section admin-push"><span className="modal-overline">NOTIFIKASI PERANTI</span><h3>Hantar makluman kepada warga sekolah</h3><p>Notifikasi diterima pada telefon atau komputer yang telah didaftarkan oleh pengguna.</p><div className="admin-push-kpis"><article><b>{pushSummary.users}</b><span>Pengguna berdaftar</span></article><article><b>{pushSummary.total}</b><span>Peranti aktif</span></article></div><div className="admin-content-form"><label>Tajuk notifikasi<input value={pushDraft.title} maxLength={100} onChange={e=>setPushDraft({...pushDraft,title:e.target.value})} placeholder="Contoh: Mesyuarat guru"/></label><label>Mesej<textarea rows={4} value={pushDraft.body} maxLength={500} onChange={e=>setPushDraft({...pushDraft,body:e.target.value})} placeholder="Tulis makluman ringkas dan jelas"/></label><div className="admin-editor-grid"><label>Sasaran<select value={pushDraft.audience} onChange={e=>setPushDraft({...pushDraft,audience:e.target.value})}><option>Semua warga</option><option>Guru</option><option>Admin</option></select></label><label>Pautan apabila ditekan<input value={pushDraft.url} onChange={e=>setPushDraft({...pushDraft,url:e.target.value})} placeholder="/ atau pautan portal"/></label></div><button className="admin-save" disabled={pushSending||!pushDraft.title.trim()||!pushDraft.body.trim()} onClick={()=>void sendPush()}>{pushSending?"Sedang menghantar…":"Hantar notifikasi"}</button></div><div className="admin-manage-list">{pushHistory.map(item=><article key={item.id}><div><small>{item.audience} · {new Date(item.createdAt).toLocaleString("ms-MY")}</small><strong>{item.title}</strong><p>{item.body}</p><span>{item.sentCount} berjaya · {item.failedCount} gagal</span></div><b className={`push-status ${item.status}`}>{item.status}</b></article>)}{!pushHistory.length&&<p>Belum ada notifikasi dihantar.</p>}</div></section>}
