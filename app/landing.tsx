...
export function LandingPortal() {
  ...
  useEffect(() => { void fetchOprIndex().catch(() => {});void loadIdentity(false).catch(()=>{setIdentity(null);setIdentityChecked(true);if(requestedModule()||requestedNotificationId())setAuthOpen(true);}); }, []);
...
function OprDutyCentre({ notify,user }:{ notify:(message:string)=>void;user:PortalIdentity|null }) {
  ...
  const loadWeek=async()=>{
    ...
    try {
      ...
      setDayStates(Object.fromEntries(sorted.map(record=>[record.reportDate,"Lengkap"])));
  ...
  const makeSummary=()=>{const actual=reports;if(!actual.length)return notify("Pilih minggu yang mempunyai laporan harian terlebih dahulu");const issues=(key:keyof DutyReport,label:string)=>{const selected=actual.filter((record)=>!["Baik","Tiada isu"].includes(String(record[key])));return selected.length?`${label}: ${selected.map((record)=>`${record.dayName} (${record[key]})`).join(", ")}.`:`${label}: Tiada isu penting dilaporkan.`;};const detailRows=actual.map((record)=>({record,details:parseDutyDetails(record)}));const locationIssues=detailRows.flatMap(({record,details})=>Object.entries(details.locationRatings||{}).filter(([,rating])=>rating!=="Baik").map(([location,rating])=>`${record.dayName}: ${location} (${rating})`));const disciplineCases=detailRows.filter(({details})=>details.disciplineControlled===false).map(({record,details})=>`${record.dayName}: ${details.disciplineCaseType||"Kes disiplin"}${details.disciplineAction?` - tindakan: ${details.disciplineAction}`:""}`);const healthTotals=detailRows.reduce((totals,{details})=>({hospital:totals.hospital+Number(details.hospitalMale||0)+Number(details.hospitalFemale||0),room:totals.room+Number(details.healthRoomMale||0)+Number(details.healthRoomFemale||0)}),{hospital:0,room:0});const monitoring=detailRows.filter(({details})=>details.monitoringAgency&&details.monitoringAgency!=="Tiada").map(({record,details})=>`${record.dayName}: ${details.monitoringAgency}${details.monitoringDetail?` - ${details.monitoringDetail}`:""}`);const notes=detailRows.flatMap(({record,details})=>[record.activityNote,details.currentIssue,record.generalNote].filter(Boolean).map((note)=>`${record.dayName}: ${note}`));setSummary([`Laporan Mingguan Minggu ${week} merangkumi ${actual.length} laporan harian yang direkodkan.`,issues("safetyStatus","Keselamatan"),issues("canteenStatus","Kantin"),locationIssues.length?`Kebersihan & keceriaan yang perlu perhatian: ${locationIssues.join("; ")}.`:"Kebersihan & keceriaan: Semua lokasi dilaporkan baik.",disciplineCases.length?`Disiplin: ${disciplineCases.join("; ")}.`:"Disiplin: Terkawal sepanjang minggu.",`Kesihatan: ${healthTotals.hospital} murid dibawa ke hospital; ${healthTotals.room} murid direkodkan di bilik kesihatan.`,monitoring.length?`Pemantauan / pelawat: ${monitoring.join("; ")}.`:"Pemantauan / pelawat: Tiada rekod.",notes.length?`Aktiviti, isu dan catatan: ${notes.join(" ")}`:"Tiada aktiviti, isu atau catatan tambahan."].join("\n\n"));};
  ...
  const makeWeeklyPdf=async()=>{
    ...
    const actual=reports;[["TAHUN",String(year)],["MINGGU",String(week)],["HARI SEKOLAH",String(actual.length)],["DISEDIAKAN",preparedBy]].forEach(([label,value],index)=>{const bx=12+index*47;pdf.setFillColor(239,247,252);pdf.roundedRect(bx,55,44,19,2,2,"F");pdf.setTextColor(45,119,165);pdf.setFontSize(6.5);pdf.text(label,bx+3,61);pdf.setTextColor(22,54,82);pdf.setFontSize(index===3?7:9);pdf.text(pdf.splitTextToSize(value,38).slice(0,2),bx+3,68);});pdf.setTextColor(22,54,82);pdf.setFontSize(8);pdf.text("REKOD HARI PERSEKOLAHAN",12,84);let y=89;actual.forEach((record,index)=>{const date=record.reportDate;pdf.setFillColor(index%2?247:239,index%2?251:247,index%2?253:252);pdf.rect(12,y,186,10,"F");pdf.setTextColor(55,75,88);pdf.setFont("helvetica","bold");pdf.setFontSize(7.2);pdf.text(record.dayName||new Intl.DateTimeFormat("ms-MY",{weekday:"long"}).format(new Date(`${date}T12:00:00`)),16,y+4.3);pdf.setFont("helvetica","normal");pdf.text(new Intl.DateTimeFormat("ms-MY",{day:"numeric",month:"long",year:"numeric"}).format(new Date(`${date}T12:00:00`)),55,y+4.3);pdf.setTextColor(45,119,165);pdf.text("Laporan harian lengkap",194,y+4.3,{align:"right"});pdf.setTextColor(100,125,135);pdf.setFontSize(6.5);pdf.text(record.preparedBy,194,y+8,{align:"right"});y+=10;});
