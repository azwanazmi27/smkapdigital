...
type Folder = "ibubapa" | "warga" | "tentang" | "schoolprofile" | "orgchart" | "announcements" | "calendar" | "directory" | "pengunjung" | "ekunjung" | "ekeberadaan" | "etempahan" | "achievement" | "epemantauan" | "oprhub" | "oprgenerator" | "oprduty" | "admin" | null;
...
export function LandingPortal() {
  ...
  const closeViewLabel = open === "oprgenerator" || open === "oprduty"
    ...
    : open === "oprhub" || open === "etempahan" || open === "ekeberadaan" || open === "achievement" || open === "epemantauan"
...
const monitoringCategory = "Lain-lain · e-Pemantauan";
...
function MonitoringCentre({ notify, user }: { notify: (message:string)=>void; user:PortalIdentity|null }) {
  ...
  const [form,setForm]=useState({title:"",date:today,time:"",locationType:"Kelas",formLevel:"",className:"",facility:"",otherLocation:"",notes:"",action:"",preparedBy:user?.name||"",preparedRole:user?.position||"",verifier:"Wan Harun Bin Wan Ali",verifierRole:"Pengetua"});
...
function AdminPanel({ notify }: { notify: (message: string) => void }) {
  ...
  const moduleNames:Array<[string,string]>=[["module_opr","Pusat OPR"],["module_ekeberadaan","E-Keberadaan & Relief"],["module_etempahan","E-Tempahan"],["module_epemantauan","e-Pemantauan"],["module_ekunjung","E-Kunjung"],["module_achievement","Arkib Kejayaan"]];
...
function BookingCentre({ notify, close, user, initialTab="dashboard" }: { notify: (message: string) => void; close: () => void; user:PortalIdentity|null; initialTab?:"dashboard"|"form" }) {
  ...
  const submit = async () => {
    ...
    try {
      const response = await fetch("/api/etempahan", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, participants: 1 }) });
...
type DutyWeekGroup = { key:string; year:number; week:number; daily:OprReport[]; weekly:OprReport[]; records:DutyReport[]; updatedAt:string };
...
function OprDutyCentre({ notify,user }:{ notify:(message:string)=>void;user:PortalIdentity|null }) {
  ...
  const loadDashboard=async(refresh=false)=>{
    ...
    let recordError=""; let driveError=""; let recordsLoaded=false;
    try {
      const response=await fetch("/api/opr-duty",{cache:"no-store"});
      const data=await response.json() as {records?:DutyReport[];error?:string};
      if(!response.ok||!data.records) throw new Error(data.error||"Rekod laporan harian tidak dapat dibaca");
      setReports(latestDutyRecordsByDate(data.records)); recordsLoaded=true;
    } catch(error){recordError=error instanceof Error?error.message:"Rekod laporan harian tidak dapat dibaca";}
    finally{
      // Paparkan rekod utama dahulu. Padanan PDF Drive boleh menyusul tanpa
      // mengosongkan dashboard pada telefon atau rangkaian yang perlahan.
    ...
    } catch(error){driveError=error instanceof Error?error.message:"PDF Google Drive tidak dapat dibaca";}
    if(recordError&&driveError)setDashboardError(`${recordError}. ${driveError}`);
    else if(!recordsLoaded&&driveError)setDashboardError(driveError);
  };
  ...
  const weekGroups=(()=>{
    ...
    for(const report of driveReports){
      ...
      const current=grouped.get(key)||{key,year:reportYear,week:reportWeek,daily:[],weekly:[],records:[],updatedAt:report.updatedAt};
    ...
    for(const record of reports){
      const key=`${record.schoolYear}-${String(record.weekNumber).padStart(2,"0")}`;
      const current=grouped.get(key)||{key,year:record.schoolYear,week:record.weekNumber,daily:[],weekly:[],records:[],updatedAt:record.updatedAt};
      current.records.push(record);if(record.updatedAt>current.updatedAt)current.updatedAt=record.updatedAt;grouped.set(key,current);
    }
    return [...grouped.values()].sort((a,b)=>b.key.localeCompare(a.key)).map((group)=>({...group,daily:latestDutyPdfByDate(group.daily).sort((a,b)=>dutyReportDate(a).localeCompare(dutyReportDate(b))),weekly:group.weekly.sort((a,b)=>b.updatedAt.localeCompare(a.updatedAt)),records:latestDutyRecordsByDate(group.records).sort((a,b)=>a.reportDate.localeCompare(b.reportDate))}));
  })();
  const selectedWeek=weekGroups.find((group)=>group.key===selectedWeekKey);
  const groupDailyCount=(group:DutyWeekGroup)=>new Set([...group.daily.map(dutyReportDate),...group.records.map((record)=>record.reportDate)]).size;
  const selectedDailyEntries=selectedWeek?Array.from(new Set([...selectedWeek.daily.map(dutyReportDate),...selectedWeek.records.map((record)=>record.reportDate)])).sort().map((date)=>({date,pdf:selectedWeek.daily.find((report)=>dutyReportDate(report)===date),record:selectedWeek.records.find((report)=>report.reportDate===date)})):[];
  ...
  return <div className="duty-centre">
    ...
    {tab==="dashboard"?<div className="duty-dashboard">
      ...
      {selectedWeek&&<div className="opr-folder-float-backdrop" onMouseDown={(event)=>event.target===event.currentTarget&&setSelectedWeekKey("")}><section className="opr-folder-float duty-week-float" role="dialog" aria-modal="true" aria-label={`Laporan Minggu ${selectedWeek.week}`}><button className="folder-float-close" onClick={()=>setSelectedWeekKey("")} aria-label="Tutup folder minggu">×</button><span className="modal-overline">FOLDER LAPORAN GURU BERTUGAS</span><h3>Minggu {selectedWeek.week}</h3><p>{selectedWeek.year} · Pilih laporan untuk membuka PDF.</p><div className="duty-week-columns"><section><h4>Laporan harian <b>{selectedDailyEntries.length}</b></h4><div className="duty-report-stack">{selectedDailyEntries.length?selectedDailyEntries.map((entry)=>{const preparedBy=entry.pdf?reportPreparer(entry.pdf):entry.record?.preparedBy||"Guru Bertugas";const day=entry.pdf?dutyReportDay(entry.pdf):entry.record?.dayName||new Intl.DateTimeFormat("ms-MY",{weekday:"long"}).format(new Date(`${entry.date}T12:00:00`));const meta=entry.pdf?dutyDailyMeta(entry.pdf,selectedWeek.week):`${new Intl.DateTimeFormat("ms-MY",{day:"numeric",month:"long",year:"numeric"}).format(new Date(`${entry.date}T12:00:00`))} | ${day} | Minggu ${selectedWeek.week} | ${preparedBy}`;return <button className="duty-report-row" key={entry.date} onClick={()=>entry.pdf?setSelectedDriveReport(entry.pdf):notify("Rekod laporan tersedia. PDF sedang dipadankan daripada Google Drive.")}><span className="duty-report-icon">▤</span><div><strong>Laporan Harian — {day}</strong><small>{meta}</small>{!entry.pdf&&<em>Rekod tersedia · PDF sedang dipadankan</em>}</div><i>{entry.pdf?"›":"…"}</i></button>; }):<p>Belum ada laporan harian.</p>}</div></section><section><h4>Laporan mingguan <b>{selectedWeek.weekly.length}</b></h4><div className="duty-report-stack">{selectedWeek.weekly.length?selectedWeek.weekly.map((report)=><button className="duty-report-row" key={report.id} onClick={()=>setSelectedDriveReport(report)}><span className="duty-report-icon">▤</span><div><strong>Laporan Mingguan — Minggu {selectedWeek.week}</strong><small>Minggu {selectedWeek.week} | {selectedWeek.year} | {reportPreparer(report)}</small></div><i>›</i></button>):<p>Belum ada PDF laporan mingguan.</p>}</div></section></div><div className="duty-week-actions"><button onClick={()=>{setForm((current)=>({...current,weekNumber:String(selectedWeek.week),schoolYear:String(selectedWeek.year)}));setWeek(selectedWeek.week);setYear(selectedWeek.year);setSelectedWeekKey("");setTab("daily");}}>Tambah laporan harian</button><button onClick={()=>{setWeek(selectedWeek.week);setYear(selectedWeek.year);setSelectedWeekKey("");setTab("weekly");}}>Jana laporan mingguan</button></div></section></div>}
