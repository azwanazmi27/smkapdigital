...
function OprDutyCentre({ notify }:{ notify:(message:string)=>void }) {
  ...
  const drawPdfHeader=(pdf:InstanceType<(typeof import("jspdf"))["jsPDF"]>,logoData:string,title:string,reference:string)=>{
  ...
  const drawPdfFooter=(pdf:InstanceType<(typeof import("jspdf"))["jsPDF"]>)=>{pdf.setFillColor(45,119,165);pdf.rect(0,289,210,8,"F");pdf.setTextColor(255,255,255);pdf.setFont("helvetica","normal");pdf.setFontSize(6.5);pdf.text("Portal Rasmi SMK Agama Pahang | Dokumen dijana secara digital",12,294);pdf.text("SMKAP",198,294,{align:"right"});};

  const makeDailyPdf=async()=>{
    const {jsPDF}=await import("jspdf"); const pdf=new jsPDF({orientation:"portrait",unit:"mm",format:"a4",compress:true}); const logo=await loadLogo();
  ...
  const makeWeeklyPdf=async()=>{
    const {jsPDF}=await import("jspdf");const pdf=new jsPDF({orientation:"portrait",unit:"mm",format:"a4",compress:true});const logo=await loadLogo();const preparedBy=form.preparedBy||reports[0]?.preparedBy||"Guru Bertugas";pdf.setProperties({title:`Laporan Mingguan Guru Bertugas Minggu ${week}`,subject:"Laporan Guru Bertugas SMK Agama Pahang",author:preparedBy,creator:"Portal Rasmi SMKAP"});drawPdfHeader(pdf,logo,"LAPORAN MINGGUAN GURU BERTUGAS",`MINGGU ${week} / ${year}`);pdf.setTextColor(22,54,82);pdf.setFont("helvetica","bold");pdf.setFontSize(14);pdf.text(`LAPORAN MINGGUAN GURU BERTUGAS - MINGGU ${week}`,12,49);
