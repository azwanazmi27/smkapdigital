...
type PortalUser={id:string;email:string;name:string;position:string;grade:string;role:string;status:string};
...
const blankPortalUser=():PortalUser=>({id:"",email:"",name:"",position:"Guru Akademik Biasa",grade:"",role:"teacher",status:"active"});
function AdminPanel({ notify }: { notify: (message: string) => void }) {
  const [token,setToken]=useState("");const [me,setMe]=useState<PortalUser|null>(null);const [users,setUsers]=useState<PortalUser[]>([]);const [loading,setLoading]=useState(true);const [error,setError]=useState("");const [query,setQuery]=useState("");const [editing,setEditing]=useState<PortalUser|null>(null);
  const api=async(method="GET",body?:PortalUser,url="/api/admin-users")=>{const response=await fetch(url,{method,headers:{...(token?{Authorization:`Bearer ${token}`}:{}) ,...(body?{"Content-Type":"application/json"}:{})},body:body?JSON.stringify(body):undefined});const data=await response.json();if(!response.ok)throw new Error(data.error||"Permintaan tidak berjaya.");return data;};
  const load=async(credential=token)=>{try{setLoading(true);setError("");const response=await fetch("/api/admin-users",{headers:{Authorization:`Bearer ${credential}`}}),data=await response.json();if(!response.ok)throw new Error(data.error||"Akses tidak dibenarkan.");setMe(data.me);setUsers(data.users);}catch(e){setError(e instanceof Error?e.message:"Akses tidak dibenarkan.");setMe(null);}finally{setLoading(false);}};
  ...
  return <div className="admin-console"><header><div><span className="modal-overline">KAWALAN PENTADBIR</span><h2 id="folder-title">Direktori warga sekolah</h2><p>{me.name} · {me.role==="super_admin"?"Super Admin":"Admin"}</p></div><button onClick={logout}>Log keluar</button></header><div className="admin-toolbar"><label>⌕<input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Cari nama, e-mel, jawatan atau gred"/></label>{me.role==="super_admin"&&<button onClick={()=>setEditing(blankPortalUser())}>＋ Tambah pengguna</button>}</div><div className="admin-user-summary"><article><b>{users.length}</b><span>Pengguna aktif</span></article><article><b>{users.filter(u=>u.role.includes("admin")).length}</b><span>Pentadbir</span></article><article><b>{users.filter(u=>u.grade).length}</b><span>Gred dilengkapkan</span></article></div>{loading?<p className="admin-state">Membaca direktori…</p>:<div className="admin-user-list">{visible.map(user=><article key={user.id}><div className="admin-avatar">{user.name.split(/\s+/).slice(0,2).map(x=>x[0]).join("")}</div><div><strong>{user.name}</strong><small>{user.email}</small><span>{user.position||"Jawatan belum ditetapkan"}{user.grade?` · ${user.grade}`:""}</span></div><b>{user.role==="super_admin"?"Super Admin":user.role==="admin"?"Admin":"Pengguna"}</b>{me.role==="super_admin"&&<div className="admin-row-actions"><button onClick={()=>setEditing({...user})}>Ubah</button><button onClick={()=>void remove(user)}>Padam</button></div>}</article>)}</div>}{editing&&<div className="admin-editor-backdrop" onMouseDown={e=>e.target===e.currentTarget&&setEditing(null)}><section className="admin-editor"><button className="folder-float-close" onClick={()=>setEditing(null)}>×</button><span className="modal-overline">{editing.id?"UBAH PENGGUNA":"PENGGUNA BAHARU"}</span><h3>Maklumat akses portal</h3><label>Nama penuh<input value={editing.name} onChange={e=>setEditing({...editing,name:e.target.value})}/></label><label>E-mel DELIMa<input type="email" value={editing.email} onChange={e=>setEditing({...editing,email:e.target.value})}/></label><label>Jawatan<input value={editing.position} onChange={e=>setEditing({...editing,position:e.target.value})}/></label><div className="admin-editor-grid"><label>Gred SSPA<select value={editing.grade} onChange={e=>setEditing({...editing,grade:e.target.value})}><option value="">Belum ditetapkan</option>{["DG5","DG6","DG7","DG8","DG9","DG10","DG12","DG13","DG14"].map(g=><option key={g}>{g}</option>)}</select></label><label>Peranan<select value={editing.role} onChange={e=>setEditing({...editing,role:e.target.value})}><option value="teacher">Pengguna</option><option value="module_admin">Admin Modul</option><option value="admin">Admin</option><option value="super_admin">Super Admin</option></select></label></div><button className="admin-save" onClick={()=>void save()}>Simpan perubahan</button></section></div>}</div>;
}
