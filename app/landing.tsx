  ...
  const prepareBundle=async(source:File)=>{
    ...
    try{
      ...
      for(let index=0;index<total;index+=1){
        ...
        pages.push({page:index+1,blob,recipient:"",achievement:"",confidence:0,status:blob.size>6_000_000?"error":"reading",error:blob.size>6_000_000?"Halaman ini melebihi 6 MB":""});
      }
      setBundleItems(pages);setReadProgress({done:0,total});
      let cursor=0,completed=0,sharedApplied=false;
      const readNext=async()=>{
        while(cursor<pages.length){
          const index=cursor++;const page=pages[index];
          if(page.status==="error"){completed+=1;setReadProgress({done:completed,total});continue;}
          try{
            ...
            updateBundleItem(page.page,{recipient:data.recipient||"",achievement:data.achievement||"",confidence:data.confidence||0,status:"review",error:""});
          ...
          completed+=1;setReadProgress({done:completed,total});
        }
      };
      await Promise.all([readNext(),readNext()]);
      notify(`${total} halaman berjaya dipecahkan. Semak nama dan pencapaian sebelum simpan.`);
