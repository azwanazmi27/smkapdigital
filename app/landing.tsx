...
function OprDutyCentre({ notify }:{ notify:(message:string)=>void }) {
  ...
  const [form,setForm]=useState({ reportDate:today,teachers:"",teacherTotal:"",teacherPresent:"",teacherAbsent:"",cleanlinessStatus:"Baik",disciplineStatus:"Tiada isu",safetyStatus:"Tiada isu",healthStatus:"Tiada isu",canteenStatus:"Tiada isu",activityNote:"",generalNote:"",preparedBy:"" });
  const weekInfo=isoWeek(form.reportDate); const dayName=new Intl.DateTimeFormat("ms-MY",{weekday:"long"}).format(new Date(`${form.reportDate}T12:00:00`));
  ...
  return <div className="duty-centre"><div className="attendance-head"><div><span className="modal-overline">PENGURUSAN · GURU BERTUGAS</span><h2 id="folder-title">Laporan guru bertugas</h2><p>Laporan harian ringkas, kemudian sistem menyediakan rumusan mengikut hari persekolahan sebenar.</p></div></div><div className="duty-tabs"><button className={tab==="daily"?"active":""} onClick={()=>setTab("daily")}>Laporan harian</button><button className={tab==="weekly"?"active":""} onClick={()=>setTab("weekly")}>Rumusan mingguan</button></div>
  {tab==="daily"?<form className="duty-form" onSubmit={saveDaily}><section><h3>Maklumat automatik</h3><div className="duty-grid three"><label>Tarikh<input type="date" value={form.reportDate} onChange={(event)=>setForm((current)=>({...current,reportDate:event.target.value}))}/></label><label>Hari<input value={dayName} readOnly/></label><label>Minggu<input value={`Minggu ${weekInfo.week}`} readOnly/></label></div><label>Nama guru bertugas<small>Masukkan nama dipisahkan dengan koma.</small><textarea value={form.teachers} onChange={(event)=>setForm((current)=>({...current,teachers:event.target.value}))} required/></label><div className="duty-grid three"><label>Bilangan guru<input type="number" min="0" value={form.teacherTotal} onChange={(event)=>setForm((current)=>({...current,teacherTotal:event.target.value}))}/></label><label>Hadir<input type="number" min="0" value={form.teacherPresent} onChange={(event)=>setForm((current)=>({...current,teacherPresent:event.target.value}))}/></label><label>Tidak hadir<input type="number" min="0" value={form.teacherAbsent} onChange={(event)=>setForm((current)=>({...current,teacherAbsent:event.target.value}))}/></label></div></section><section><h3>Semakan pantas</h3><div className="duty-grid">{statusField("Kebersihan & keceriaan","cleanlinessStatus")}{statusField("Disiplin","disciplineStatus")}{statusField("Keselamatan","safetyStatus")}{statusField("Kesihatan murid","healthStatus")}{statusField("Kantin","canteenStatus")}</div><label>Aktiviti, lawatan atau peristiwa semasa<textarea value={form.activityNote} onChange={(event)=>setForm((current)=>({...current,activityNote:event.target.value}))} placeholder="Kosongkan jika tiada"/></label><label>Catatan penting sahaja<textarea value={form.generalNote} onChange={(event)=>setForm((current)=>({...current,generalNote:event.target.value}))} placeholder="Kosongkan jika tiada"/></label><label>Disediakan oleh<input value={form.preparedBy} onChange={(event)=>setForm((current)=>({...current,preparedBy:event.target.value}))} required/></label></section><button className="duty-primary" disabled={saving}>{saving?"Menyimpan…":"Simpan laporan harian"}</button></form>:
