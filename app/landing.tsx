...
function OprDutyCentre({ notify,user }:{ notify:(message:string)=>void;user:PortalIdentity|null }) {
  ...
  const weekGroups=(()=>{
    ...
    return [...grouped.values()].sort((a,b)=>b.key.localeCompare(a.key)).map((group)=>({...group,daily:latestDutyPdfByDate(group.daily).sort((a,b)=>a.updatedAt.localeCompare(b.updatedAt)),weekly:group.weekly.sort((a,b)=>b.updatedAt.localeCompare(a.updatedAt))}));
  })();
  const selectedWeek=weekGroups.find((group)=>group.key===selectedWeekKey);
  const dailyCount=weekGroups.reduce((total,group)=>total+group.daily.length,0);
