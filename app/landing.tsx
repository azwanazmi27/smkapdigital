...
export function LandingPortal() {
  ...
  const [skasAccess,setSkasAccess]=useState(true);
  ...
  const loadIdentity=async(showWelcome=false)=>{const response=await fetch("/api/admin-users?resource=me",{cache:"no-store"}),data=await response.json();if(!response.ok)throw new Error(data.error||"Log masuk tidak berjaya.");const accessResponse=await fetch("/api/skas-access",{cache:"no-store"}),accessData=await accessResponse.json() as{allowed?:boolean};const allowed=accessResponse.ok&&accessData.allowed!==false;setSkasAccess(allowed);setIdentity(data.me);setIdentityChecked(true);setAuthOpen(false);void loadStaffAnnouncements();const target=requestedModule();if(target==="skas"&&!allowed){alert("Anda tidak mempunyai kebenaran.");setOpen("warga");}else if(target)setOpen(target);else if(showWelcome&&!requestedNotificationId())setOpen("warga");if(showWelcome&&!requestedNotificationId())setWelcome(true);await openRequestedNotification();};
  ...
  const openSubmodule=(folder:Folder,title:string)=>{if(folder==="skas"&&!skasAccess){alert("Anda tidak mempunyai kebenaran.");return;}setOpening(title);setOpen(folder);finishOpening();};
...
function AdminPanel({ notify }: { notify: (message: string) => void }) {
  ...
  const skasEnabled=(userId:string)=>permissions.find(item=>item.userId===userId&&item.moduleKey==="skas")?.enabled!==0;
  const toggleSkasAccess=async(user:PortalUser)=>{try{await api("PUT",{userId:user.id,enabled:!skasEnabled(user.id)},"/api/skas-access");notify("Kebenaran Pusat SK@S dikemas kini");await load();}catch(e){notify(e instanceof Error?e.message:"Kebenaran SK@S tidak dapat disimpan")}};
...
const exportJson=async(kind:"ekeberadaan"|"etempahan")=>{setExporting(kind);try{let payload:unknown;if(kind==="ekeberadaan"){const response=await fetch("/api/ekeberadaan?resource=all",{cache:"no-store"});payload=await response.json();if(!response.ok)throw new Error("Data E-Keberadaan tidak dapat dieksport");}else{const year=new Date().getFullYear(),all:unknown[]=[];for(let month=0;month<12;month+=1){const from=`${year}-${String(month+1).padStart(2,"0")}-01`,end=new Date(year,month+1,0),to=`${year}-${String(month+1).padStart(2,"0")}-${String(end.getDate()).padStart(2,"0")}`;const response=await fetch(`/api/etempahan?from=${from}&to=${to}`,{cache:"no-store"}),data=await response.json();if(response.ok&&Array.isArray(data.bookings))all.push(...data.bookings);}payload={exportedAt:new Date().toISOString(),year,bookings:all};}saveDownload(new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),`${kind}-SMKAP-${new Date().toISOString().slice(0,10)}.json`);notify(`Data ${kind} berjaya dimuat turun`);}catch(e){notify(e instanceof Error?e.message:"Eksport tidak berjaya");}finally{setExporting("");}};
  ...
  const nav=[["dashboard","Dashboard"],["users","Pengguna"],["expertise","Kepakaran Guru"],["access","Akses Admin"],["content","Tentang Sekolah"],["announcements","Pengumuman"],["notifications","Notifikasi"],["calendar","Takwim"],["modules","Modul"],["export","Eksport & Sandaran"],["trash","Tong Sampah"],["logs","Log Aktiviti"]];
  ...
  return <div className="admin-console"><header><div><span className="modal-overline">KAWALAN PENTADBIR</span><h2 id="folder-title">Pusat Pentadbiran Portal</h2><p>{me.name} · {me.role==="super_admin"?"Super Admin":"Admin"}</p></div><button onClick={logout}>Log keluar</button></header>
    ...
    {tab==="access"&&<><section className="admin-section"><span className="modal-overline">AKSES PUSAT SK@S</span><h3>Tentukan pengguna yang boleh membuka Pusat SK@S</h3><p>Semua pengguna dibenarkan secara lalai. Matikan akses hanya untuk pengguna yang tidak dibenarkan.</p><div className="admin-access-list skas-user-access">{users.filter(user=>user.status==="active").map(user=><article key={user.id}><div><strong>{user.name}</strong><small>{user.email} · {user.role==="teacher"?"Pengguna":user.role==="admin"?"Admin":"Super Admin"}</small></div><label><input type="checkbox" checked={skasEnabled(user.id)} disabled={user.role==="super_admin"} onChange={()=>void toggleSkasAccess(user)}/><span>{skasEnabled(user.id)?"Dibenarkan":"Tiada akses"}</span></label></article>)}</div></section><section className="admin-section"><span className="modal-overline">AKSES ADMIN</span><h3>Hadkan fungsi setiap admin</h3><p>Super Admin kekal mempunyai semua kuasa. Pilih modul yang boleh diurus oleh setiap Admin.</p><div className="admin-access-list">{users.filter(u=>u.role==="admin").map(user=><article key={user.id}><div><strong>{user.name}</strong><small>{user.email}</small></div><div>{accessModules.map(([key,label])=><label key={key}><input type="checkbox" checked={permissions.some(x=>x.userId===user.id&&x.moduleKey===key&&x.enabled)} onChange={()=>void togglePermission(user,key)}/><span>{label}</span></label>)}</div></article>)}{!users.some(u=>u.role==="admin")&&<p>Belum ada akaun Admin biasa.</p>}</div></section></>}
