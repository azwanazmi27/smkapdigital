...
function AttendanceCentre({ notify, user }: { notify: (message: string) => void; user: PortalIdentity | null }) {
  ...
  const deleteRecord = async (record: Absence) => {
    if (!isAdmin || !confirm(`Buang rekod ketidakhadiran ${record.teacherName}?`)) return;
    try {
      const response = await fetch(`/api/ekeberadaan?resource=absences&id=${encodeURIComponent(record.id)}`, { method: "DELETE" });
      const data = await response.json() as { error?: string };
      if (!response.ok) throw new Error(data.error || "Rekod tidak dapat dibuang");
      setRecords((current) => current.filter((item) => item.id !== record.id));
      notify("Rekod ketidakhadiran telah dibuang");
    } catch (issue) { notify(issue instanceof Error ? issue.message : "Rekod tidak dapat dibuang"); }
  };
