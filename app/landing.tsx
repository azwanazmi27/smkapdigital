...
export function LandingPortal() {
  ...
  const [staffAnnouncements,setStaffAnnouncements]=useState<Array<{id:string;title:string;body:string;publishedAt:string;endsAt:string}>>([]);
  ...
  const openFolder=(folder:typeof folders[number])=>{if(folder.id==="warga"&&!identity){setAuthOpen(true);return;}setOpen(folder.id);if(folder.id==="warga"&&identity&&staffAnnouncements.length)setStaffAnnouncementOpen(true);};
  ...
  const updateProfile=async(profile:{name:string;position:string;grade:string})=>{const token=sessionStorage.getItem("smkap_google_token")||"";const response=await fetch("/api/admin-users?resource=profile",{method:"PUT",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:JSON.stringify(profile)}),data=await response.json();if(!response.ok)throw new Error(data.error||"Profil tidak dapat dikemas kini.");setIdentity(current=>current?{...current,...profile}:current);notify("Profil berjaya dikemas kini");};
  const logout=()=>{sessionStorage.removeItem("smkap_google_token");window.google?.accounts.id.disableAutoSelect?.();setIdentity(null);setWelcome(false);setProfileOpen(false);setAuthOpen(false);setOpen(null);notify("Anda telah log keluar");};
  ...
  return <main className="landing-shell">
    ...
    {welcome&&identity&&<LoginWelcome user={identity} close={()=>{setWelcome(false);if(staffAnnouncements.length)window.setTimeout(()=>setStaffAnnouncementOpen(true),180)}}/>}
    {staffAnnouncementOpen&&staffAnnouncements.length>0&&<StaffAnnouncementPopup items={staffAnnouncements} close={()=>setStaffAnnouncementOpen(false)}/>}
...
type PublicContent={settings:Record<string,string>;announcements:Array<{id:string;title:string;body:string;audience:string;publishedAt:string}>;events:Array<{id:string;title:string;eventDate:string;endDate:string;category:string;details:string}>;leaders:Array<{id:string;name:string;position:string;grade:string}>};
function usePublicContent(){const [data,setData]=useState<PublicContent|null>(null);useEffect(()=>{fetch("/api/portal-content",{cache:"no-store"}).then(response=>response.json()).then(setData).catch(()=>setData({settings:{},announcements:[],events:[],leaders:[]}));},[]);return data;}
function SchoolProfile(){const data=usePublicContent(),s=data?.settings||{};return <div className="school-info-view"><figure className="school-profile-hero"><img src="/kampus-smkap-panorama.jpeg" alt="Bangunan SMK Agama Pahang"/><figcaption><img src="/logo-smkap.png" alt="Logo SMK Agama Pahang"/><div><span>TENTANG SEKOLAH</span><strong>{s.school_name||"SMK Agama Pahang"}</strong><small>{s.school_motto||"Berilmu, Bertakwa"}</small></div></figcaption></figure><h2 id="folder-title">{s.school_name||"SMK Agama Pahang"}</h2><p className="school-lead">Institusi pendidikan menengah kebangsaan agama di Muadzam Shah yang menggabungkan kecemerlangan ilmu, penghayatan agama dan pembentukan sahsiah.</p><div className="school-facts">{[["Kod sekolah",s.school_code],["Tarikh ditubuhkan",s.school_founded],["Kategori",s.school_type],["Gred",s.school_grade],["Alamat",s.school_address],["Hubungi",`${s.school_phone||""} · ${s.school_email||""}`]].map(([label,value])=><article key={label}><small>{label}</small><strong>{value||"Akan dikemas kini"}</strong></article>)}</div><section><span>SEJARAH RINGKAS</span><p>{s.school_history}</p></section><div className="school-direction"><article><span>VISI</span><strong>{s.school_vision}</strong></article><article><span>MISI</span><strong>{s.school_mission}</strong></article><article><span>COGAN KATA</span><strong>{s.school_motto||"Berilmu, Bertakwa"}</strong></article></div><small className="school-source-note">Maklumat asas disusun daripada sumber awam sekolah dan boleh dikemas kini oleh Admin.</small></div>}
function OrganizationChart(){const data=usePublicContent(),rank=(position:string)=>position.toLowerCase().includes("pengetua")&&!position.toLowerCase().includes("penolong")?0:position.toLowerCase().includes("penolong kanan")?1:2,leaders=[...(data?.leaders||[])].sort((a,b)=>rank(a.position)-rank(b.position)||a.position.localeCompare(b.position));return <div className="org-view"><span className="modal-overline">STRUKTUR KEPIMPINAN</span><h2 id="folder-title">Carta organisasi</h2><p>Dijana secara automatik daripada nama dan jawatan aktif dalam senarai pengguna.</p><div className="org-tree">{leaders.map((person,index)=><article key={person.id} className={`org-level-${rank(person.position)}`}><span>{person.name.split(/\s+/).slice(0,2).map(x=>x[0]).join("")}</span><div><small>{person.position}</small><strong>{person.name}</strong>{person.grade&&<b>{person.grade}</b>}</div>{index===0&&<i></i>}</article>)}{!leaders.length&&<p>Maklumat kepimpinan belum dilengkapkan oleh Admin.</p>}</div></div>}
function PublicAnnouncements(){const data=usePublicContent();return <div className="public-list-view"><span className="modal-overline">HEBAHAN SEKOLAH</span><h2 id="folder-title">Pengumuman terkini</h2><p>Makluman rasmi yang diterbitkan oleh pihak sekolah.</p><div>{data?.announcements.length?data.announcements.map(item=><article key={item.id}><span>{new Date(`${item.publishedAt}T12:00:00`).toLocaleDateString("ms-MY",{day:"2-digit",month:"short"})}</span><div><small>{item.audience}</small><strong>{item.title}</strong><p>{item.body}</p></div></article>):<p className="directory-empty">Belum ada pengumuman diterbitkan.</p>}</div></div>}
...
type PortalUser={id:string;email:string;name:string;position:string;grade:string;role:string;status:string;showDirectory?:boolean};
...
type PortalAnnouncement={id:string;title:string;body:string;audience:string;publishedAt:string;endsAt:string};
...
const blankPortalUser=():PortalUser=>({id:"",email:"",name:"",position:"Guru Akademik Biasa",grade:"",role:"teacher",status:"active",showDirectory:true});
function AdminPanel({ notify }: { notify: (message: string) => void }) {
  const today=new Date().toISOString().slice(0,10);
  const [token,setToken]=useState("");const [me,setMe]=useState<PortalUser|null>(null);const [users,setUsers]=useState<PortalUser[]>([]);const [trash,setTrash]=useState<PortalUser[]>([]);const [logs,setLogs]=useState<AdminLog[]>([]);const [permissions,setPermissions]=useState<AdminPermission[]>([]);const [settings,setSettings]=useState<Record<string,string>>({});const [announcements,setAnnouncements]=useState<PortalAnnouncement[]>([]);const [events,setEvents]=useState<PortalEvent[]>([]);const [announcementDraft,setAnnouncementDraft]=useState({title:"",body:"",audience:"Semua",publishedAt:today,endsAt:today});const [eventDraft,setEventDraft]=useState({title:"",eventDate:today,endDate:today,category:"Sekolah",details:""});const [tab,setTab]=useState("dashboard");const [loading,setLoading]=useState(true);const [error,setError]=useState("");const [query,setQuery]=useState("");const [editing,setEditing]=useState<PortalUser|null>(null);
  const api=async(method="GET",body?:unknown,url="/api/admin-users")=>{const response=await fetch(url,{method,headers:{...(token?{Authorization:`Bearer ${token}`}:{}) ,...(body?{"Content-Type":"application/json"}:{})},body:body?JSON.stringify(body):undefined});const data=await response.json();if(!response.ok)throw new Error(data.error||"Permintaan tidak berjaya.");return data;};
  const load=async(credential=token)=>{try{setLoading(true);setError("");const headers={Authorization:`Bearer ${credential}`},[adminResponse,contentResponse]=await Promise.all([fetch("/api/admin-users",{headers}),fetch("/api/portal-content?view=admin",{headers,cache:"no-store"})]),data=await adminResponse.json(),content=await contentResponse.json();if(!adminResponse.ok)throw new Error(data.error||"Akses tidak dibenarkan.");setMe(data.me);setUsers(data.users||[]);setTrash(data.trash||[]);setLogs(data.logs||[]);setPermissions(data.permissions||[]);setSettings({...data.settings,...(content.settings||{})});setAnnouncements(content.announcements||[]);setEvents(content.events||[]);}catch(e){setError(e instanceof Error?e.message:"Akses tidak dibenarkan.");setMe(null);}finally{setLoading(false);}};
  useEffect(()=>{const saved=sessionStorage.getItem("smkap_google_token")||"";if(saved){setToken(saved);void load(saved);return;}let cancelled=false;const start=async()=>{try{const config=await fetch("/api/admin-users?resource=config").then(r=>r.json());if(!document.querySelector('script[src="https://accounts.google.com/gsi/client"]')){const s=document.createElement("script");s.src="https://accounts.google.com/gsi/client";s.async=true;document.head.appendChild(s);}for(let i=0;i<40&&!window.google;i++)await new Promise(r=>setTimeout(r,100));if(cancelled||!window.google)return;window.google.accounts.id.initialize({client_id:config.clientId,callback:({credential})=>{sessionStorage.setItem("smkap_google_token",credential);setToken(credential);void load(credential);}});const el=document.getElementById("google-admin-signin");if(el){el.innerHTML="";window.google.accounts.id.renderButton(el,{theme:"outline",size:"large",text:"continue_with",shape:"pill",width:280});}setLoading(false);}catch{setError("Butang Google tidak dapat disediakan sekarang.");setLoading(false);}};void start();return()=>{cancelled=true};},[]);
  const save=async()=>{if(!editing)return;try{await api(editing.id?"PUT":"POST",editing);setEditing(null);notify("Maklumat pengguna berjaya disimpan");await load();}catch(e){notify(e instanceof Error?e.message:"Tidak dapat disimpan")}};
  ...
  const saveProfileSettings=async()=>{try{await api("PUT",{settings},"/api/portal-content");notify("Profil sekolah berjaya dikemas kini");await load();}catch(e){notify(e instanceof Error?e.message:"Profil tidak dapat disimpan")}};
  ...
  const saveAnnouncement=async()=>{try{await api("POST",{kind:"announcement",...announcementDraft},"/api/portal-content");setAnnouncementDraft({title:"",body:"",audience:"Semua",publishedAt:today,endsAt:today});notify("Pengumuman berjaya diterbitkan");await load();}catch(e){notify(e instanceof Error?e.message:"Pengumuman tidak dapat diterbitkan")}};
  const saveEvent=async()=>{try{await api("POST",{kind:"event",...eventDraft},"/api/portal-content");setEventDraft({title:"",eventDate:today,endDate:today,category:"Sekolah",details:""});notify("Aktiviti takwim berjaya disimpan");await load();}catch(e){notify(e instanceof Error?e.message:"Aktiviti tidak dapat disimpan")}};
  const deleteContent=async(kind:"announcement"|"event",id:string)=>{if(!confirm("Padam rekod ini?"))return;try{await api("DELETE",undefined,`/api/portal-content?kind=${kind}&id=${encodeURIComponent(id)}`);notify("Rekod telah dipadam");await load();}catch(e){notify(e instanceof Error?e.message:"Rekod tidak dapat dipadam")}};
  ...
  const logout=()=>{sessionStorage.removeItem("smkap_google_token");window.google?.accounts.id.disableAutoSelect();setToken("");setMe(null);setUsers([]);location.reload();};
  ...
  const nav=[["dashboard","Dashboard"],["users","Pengguna"],["access","Akses Admin"],["content","Tentang Sekolah"],["announcements","Pengumuman"],["calendar","Takwim"],["modules","Modul"],["trash","Tong Sampah"],["logs","Log Aktiviti"]];
  ...
  return <div className="admin-console"><header><div><span className="modal-overline">KAWALAN PENTADBIR</span><h2 id="folder-title">Pusat Pentadbiran Portal</h2><p>{me.name} · {me.role==="super_admin"?"Super Admin":"Admin"}</p></div><button onClick={logout}>Log keluar</button></header>
    ...
    {tab==="users"&&<><div className="admin-toolbar"><label>⌕<input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Cari nama, e-mel, jawatan atau gred"/></label>{me.role==="super_admin"&&<button onClick={()=>setEditing(blankPortalUser())}>＋ Tambah pengguna</button>}</div>{loading?<p className="admin-state">Membaca direktori…</p>:<div className="admin-user-list">{visible.map(user=><article key={user.id}><div className="admin-avatar">{user.name.split(/\s+/).slice(0,2).map(x=>x[0]).join("")}</div><div><strong>{user.name}</strong><small>{user.email}</small><span>{user.position||"Jawatan belum ditetapkan"}{user.grade?` · ${user.grade}`:""} · {user.showDirectory===false?"Tidak dipapar":"Dipapar dalam direktori"}</span></div><b>{user.status==="inactive"?"Tidak aktif":user.role==="super_admin"?"Super Admin":user.role==="admin"?"Admin":"Pengguna"}</b>{me.role==="super_admin"&&<div className="admin-row-actions"><button onClick={()=>setEditing({...user})}>Ubah</button><button onClick={()=>void remove(user)}>Padam</button></div>}</article>)}</div>}</>}
    ...
    {tab==="content"&&<section className="admin-section admin-settings"><span className="modal-overline">TENTANG SEKOLAH</span><h3>Kemas kini profil rasmi sekolah</h3><p>Carta organisasi tidak perlu ditaip di sini. Ia dibina automatik daripada jawatan Pengetua, Penolong Kanan dan Guru Kanan Mata Pelajaran dalam senarai pengguna.</p><div className="admin-editor-grid"><label>Nama rasmi sekolah<input value={settings.school_name||""} onChange={e=>setSettings({...settings,school_name:e.target.value})}/></label><label>Kod sekolah<input value={settings.school_code||""} onChange={e=>setSettings({...settings,school_code:e.target.value})}/></label><label>Jenis / kategori<input value={settings.school_type||""} onChange={e=>setSettings({...settings,school_type:e.target.value})}/></label><label>Gred sekolah<input value={settings.school_grade||""} onChange={e=>setSettings({...settings,school_grade:e.target.value})}/></label><label>Tarikh ditubuhkan<input value={settings.school_founded||""} onChange={e=>setSettings({...settings,school_founded:e.target.value})}/></label><label>Cogan kata<input value={settings.school_motto||""} onChange={e=>setSettings({...settings,school_motto:e.target.value})}/></label><label>E-mel sekolah<input value={settings.school_email||""} onChange={e=>setSettings({...settings,school_email:e.target.value})}/></label><label>Nombor telefon<input value={settings.school_phone||""} onChange={e=>setSettings({...settings,school_phone:e.target.value})}/></label></div><label>Alamat sekolah<input value={settings.school_address||""} onChange={e=>setSettings({...settings,school_address:e.target.value})}/></label><label>Sejarah ringkas<textarea rows={5} value={settings.school_history||""} onChange={e=>setSettings({...settings,school_history:e.target.value})}/></label><label>Visi<textarea rows={3} value={settings.school_vision||""} onChange={e=>setSettings({...settings,school_vision:e.target.value})}/></label><label>Misi<textarea rows={3} value={settings.school_mission||""} onChange={e=>setSettings({...settings,school_mission:e.target.value})}/></label><label>Teks kaki portal<input value={settings.footer_notice||""} onChange={e=>setSettings({...settings,footer_notice:e.target.value})}/></label><button className="admin-save" onClick={()=>void saveProfileSettings()}>Simpan profil sekolah</button></section>}
    {tab==="announcements"&&<section className="admin-section"><span className="modal-overline">PENGUMUMAN</span><h3>Terbitkan hebahan sekolah</h3><p>Pilih “Guru & Staf” untuk memaparkan pengumuman sebagai popup selepas login dan setiap kali ruang Guru & Staf dibuka.</p><div className="admin-content-form"><label>Tajuk pengumuman<input value={announcementDraft.title} onChange={e=>setAnnouncementDraft({...announcementDraft,title:e.target.value})}/></label><div className="admin-editor-grid"><label>Sasaran<select value={announcementDraft.audience} onChange={e=>setAnnouncementDraft({...announcementDraft,audience:e.target.value})}><option>Semua</option><option>Guru & Staf</option><option>Ibu Bapa / Penjaga</option><option>Murid</option></select></label><label>Tarikh mula<input type="date" value={announcementDraft.publishedAt} onChange={e=>setAnnouncementDraft({...announcementDraft,publishedAt:e.target.value,endsAt:e.target.value>announcementDraft.endsAt?e.target.value:announcementDraft.endsAt})}/></label><label>Tarikh tamat<input type="date" min={announcementDraft.publishedAt} value={announcementDraft.endsAt} onChange={e=>setAnnouncementDraft({...announcementDraft,endsAt:e.target.value})}/></label></div><label>Maklumat<textarea rows={5} value={announcementDraft.body} onChange={e=>setAnnouncementDraft({...announcementDraft,body:e.target.value})}/></label><button className="admin-save" onClick={()=>void saveAnnouncement()}>Terbitkan pengumuman</button></div><div className="admin-manage-list">{announcements.map(item=><article key={item.id}><div><small>{item.audience} · {new Date(`${item.publishedAt}T12:00:00`).toLocaleDateString("ms-MY")} – {new Date(`${item.endsAt||item.publishedAt}T12:00:00`).toLocaleDateString("ms-MY")}</small><strong>{item.title}</strong><p>{item.body}</p></div><button onClick={()=>void deleteContent("announcement",item.id)}>Padam</button></article>)}{!announcements.length&&<p>Belum ada pengumuman.</p>}</div></section>}
