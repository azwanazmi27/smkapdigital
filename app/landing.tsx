...
function AIStatusList(){const [online,setOnline]=useState(false);useEffect(()=>{let active=true;fetch("/api/ai/health",{cache:"no-store"}).then(async r=>{if(!r.ok)throw new Error("Status AI tidak tersedia");return r.json() as Promise<{providers?:Record<string,string>}>;}).then(data=>{if(active)setOnline(Object.values(data.providers||{}).some(status==="online"));}).catch(()=>{if(active)setOnline(false);});return()=>{active=false;};},[]);return <div className={`bang-wan-status ${online?"online":"offline"}`} role="status" aria-label={`Bang Wan ${online?"Online":"Offline"}`}><img className="bang-wan-robot" src="/bang-wan-opr.png" alt="" aria-hidden="true"/><div><strong>Bang Wan</strong><b><i aria-hidden="true"/>{online?"Online":"Offline"}</b></div><small>{online?"Sentiasa sedia membantu":"Cuba semula sebentar lagi"}</small></div>}
