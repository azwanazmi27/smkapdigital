...
type PortalAnnouncement={id:string;title:string;body:string;audience:string;publishedAt:string};
...
function AdminPanel({ notify }: { notify: (message: string) => void }) {
  ...
  const saveAnnouncement=async()=>{try{await api("POST",{kind:"announcement",...announcementDraft},"/api/portal-content");setAnnouncementDraft({title:"",body:"",audience:"Semua",publishedAt:today});notify("Pengumuman berjaya diterbitkan");await load();}catch(e){notify(e instanceof Error?e.message:"Pengumuman tidak dapat diterbitkan")}};
  ...
  return <div className="admin-console"><header><div><span className="modal-overline">KAWALAN PENTADBIR</span><h2 id="folder-title">Pusat Pentadbiran Portal</h2><p>{me.name} · {me.role==="super_admin"?"Super Admin":"Admin"}</p></div><button onClick={logout}>Log keluar</button></header>
    ...
    {tab==="announcements"&&<section className="admin-section"><span className="modal-overline">PENGUMUMAN</span><h3>Terbitkan hebahan sekolah</h3><div className="admin-content-form"><label>Tajuk pengumuman<input value={announcementDraft.title} onChange={e=>setAnnouncementDraft({...announcementDraft,title:e.target.value})}/></label><div className="admin-editor-grid"><label>Sasaran<select value={announcementDraft.audience} onChange={e=>setAnnouncementDraft({...announcementDraft,audience:e.target.value})}><option>Semua</option><option>Guru & Staf</option><option>Ibu Bapa / Penjaga</option><option>Murid</option></select></label><label>Tarikh diterbitkan<input type="date" value={announcementDraft.publishedAt} onChange={e=>setAnnouncementDraft({...announcementDraft,publishedAt:e.target.value})}/></label></div><label>Maklumat<textarea rows={5} value={announcementDraft.body} onChange={e=>setAnnouncementDraft({...announcementDraft,body:e.target.value})}/></label><button className="admin-save" onClick={()=>void saveAnnouncement()}>Terbitkan pengumuman</button></div><div className="admin-manage-list">{announcements.map(item=><article key={item.id}><div><small>{item.audience} · {new Date(item.publishedAt).toLocaleDateString("ms-MY")}</small><strong>{item.title}</strong><p>{item.body}</p></div><button onClick={()=>void deleteContent("announcement",item.id)}>Padam</button></article>)}{!announcements.length&&<p>Belum ada pengumuman.</p>}</div></section>}
