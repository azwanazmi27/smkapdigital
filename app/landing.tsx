...
function OprGenerator({ notify, close, user, initialCategoryGroup="", initialCategory="" }: { notify: (message: string) => void; close: () => void; user:PortalIdentity|null; initialCategoryGroup?:string;initialCategory?:string }) {
  ...
  return <div className="opr-generator">
    ...
    {!preview ? <form className="generator-form" onSubmit={(event) => { event.preventDefault(); if (complete) void preparePreview(); }}>
      ...
      <AIUsageNote/><button type="button" className="generator-ai" onClick={enhance} disabled={enhancing}><span>✦</span><span>{enhancing ? "AI sedang menulis..." : "Jana pelaksanaan, objektif & hasil dengan AI"}<small>AI mengekalkan fakta asal dan mengemaskan ketiga-tiga bahagian</small></span></button>
