...
function OrganizationChart(){const data=usePublicContent(),rank=(position:string)=>position.toLowerCase().includes("pengetua")&&!position.toLowerCase().includes("penolong")?0:position.toLowerCase().includes("penolong kanan")?1:2,order=(position:string)=>{const p=position.toLowerCase();return ["pentadbiran","hal ehwal murid","kokurikulum","tingkatan enam","bahasa","sains & matematik","kemanusiaan","teknik & vokasional","pendidikan islam"].findIndex(term=>p.includes(term))},leaders=[...(data?.leaders||[])].sort((a,b)=>rank(a.position)-rank(b.position)||(order(a.position)<0?99:order(a.position))-(order(b.position)<0?99:order(b.position))||a.position.localeCompare(b.position)),card=(person:typeof leaders[number])=><article key={person.id}>{person.imageDataUrl?<img src={person.imageDataUrl} alt={`Gambar ${person.name}`}/>:<span>{person.name.split(/\s+/).slice(0,2).map(x=>x[0]).join("")}</span>}<div><small>{person.position}</small><strong>{person.name}</strong>{person.grade&&<b>{person.grade}</b>}</div></article>;return <div className="org-view"><span className="modal-overline">STRUKTUR KEPIMPINAN</span><h2 id="folder-title">Carta organisasi</h2><p>Dijana secara automatik daripada nama, jawatan dan gambar yang dikemas kini oleh Super Admin.</p><div className="org-tree">{[0,1,2].map(level=>{const members=leaders.filter(person=>rank(person.position)===level);return members.length?<section key={level} className={`org-tier org-tier-${level}`}><h3>{level===0?"Pengetua":level===1?"Barisan Penolong Kanan":"Guru Kanan Mata Pelajaran"}</h3><div>{members.map(card)}</div></section>:null})}{!leaders.length&&<p>Maklumat kepimpinan belum dilengkapkan oleh Admin.</p>}</div></div>}
