...
function AdminPanel({ notify }: { notify: (message: string) => void }) {
  ...
  const load=async(credential=token)=>{try{setLoading(true);setError("");const response=await fetch("/api/admin-users",{headers:{Authorization:`Bearer ${credential}`}}),data=await response.json();if(!response.ok)throw new Error(data.error||"Akses tidak dibenarkan.");setMe(data.me);setUsers(data.users||[]);setTrash(data.trash||[]);setLogs(data.logs||[]);setPermissions(data.permissions||[]);setSettings(data.settings||{});}catch(e){setError(e instanceof Error?e.message:"Akses tidak dibenarkan.");setMe(null);}finally{setLoading(false);}};
  ...
  const saveSettings=async()=>{try{await api("PUT",{settings},"/api/admin-users?resource=settings");notify("Tetapan portal berjaya disimpan");await load();}catch(e){notify(e instanceof Error?e.message:"Tetapan tidak dapat disimpan")}};
  ...
  const nav=[["dashboard","Dashboard"],["users","Pengguna"],["access","Akses Admin"],["content","Kandungan"],["modules","Modul"],["trash","Tong Sampah"],["logs","Log Aktiviti"]];
  ...
  return <div className="admin-console"><header><div><span className="modal-overline">KAWALAN PENTADBIR</span><h2 id="folder-title">Pusat Pentadbiran Portal</h2><p>{me.name} · {me.role==="super_admin"?"Super Admin":"Admin"}</p></div><button onClick={logout}>Log keluar</button></header>
    ...
    {tab==="content"&&<section className="admin-section admin-settings"><span className="modal-overline">KANDUNGAN AWAM</span><h3>Maklumat hubungan sekolah</h3><div className="admin-editor-grid"><label>E-mel sekolah<input value={settings.school_email||""} onChange={e=>setSettings({...settings,school_email:e.target.value})}/></label><label>Nombor telefon<input value={settings.school_phone||""} onChange={e=>setSettings({...settings,school_phone:e.target.value})}/></label></div><label>Alamat sekolah<input value={settings.school_address||""} onChange={e=>setSettings({...settings,school_address:e.target.value})}/></label><label>Teks kaki portal<input value={settings.footer_notice||""} onChange={e=>setSettings({...settings,footer_notice:e.target.value})}/></label><button className="admin-save" onClick={()=>void saveSettings()}>Simpan kandungan portal</button></section>}
    {tab==="modules"&&<section className="admin-section"><span className="modal-overline">KAWALAN MODUL</span><h3>Buka atau tutup modul sementara</h3><p>Gunakan ketika penyelenggaraan. Data modul tidak dipadam.</p><div className="admin-module-list">{moduleNames.map(([key,label])=><button key={key} disabled={me.role!=="super_admin"} onClick={()=>setSettings({...settings,[key]:settings[key]==="disabled"?"enabled":"disabled"})}><span><strong>{label}</strong><small>{settings[key]==="disabled"?"Pengguna tidak boleh membuka modul":"Modul tersedia kepada pengguna"}</small></span><i className={settings[key]==="disabled"?"off":""}></i></button>)}</div>{me.role==="super_admin"&&<button className="admin-save" onClick={()=>void saveSettings()}>Simpan status modul</button>}</section>}
