...
function AIStatusList(){const [items,setItems]=useState<Record<string,string>>({});useEffect(()=>{fetch("/api/ai/health",{cache:"no-store"}).then(r=>r.json()).then(data=>setItems(data.providers||{})).catch(()=>setItems({}));},[]);return <div className="ai-status-list" aria-label="Status perkhidmatan AI">{Object.entries(aiLabels).map(([key,label])=><span key={key} className={items[key]==="online"?"online":"offline"}><i></i>{label}<b>{items[key]==="online"?"Online":"Offline"}</b></span>)}</div>}
