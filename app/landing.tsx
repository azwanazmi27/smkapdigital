...
function AdminPanel({ notify }: { notify: (message: string) => void }) {
  ...
  useEffect(()=>{const saved=sessionStorage.getItem("smkap_google_token")||"";if(saved){setToken(saved);void load(saved);return;}let cancelled=false;const start=async()=>{try{const config=await fetch("/api/admin-users?resource=config").then(r=>r.json());if(!document.querySelector('script[src="https://accounts.google.com/gsi/client"]')){const s=document.createElement("script");s.src="https://accounts.google.com/gsi/client";s.async=true;document.head.appendChild(s);}for(let i=0;i<40&&!window.google;i++)await new Promise(r=>setTimeout(r,100));if(cancelled||!window.google)return;window.google.accounts.id.initialize({client_id:config.clientId,callback:({credential})=>{sessionStorage.setItem("smkap_google_token",credential);setToken(credential);void load(credential);}});const el=document.getElementById("google-admin-signin");if(el){el.innerHTML="";window.google.accounts.id.renderButton(el,{theme:"outline",size:"large",text:"continue_with",shape:"pill",width:280});}}catch{setError("Butang Google tidak dapat disediakan sekarang.");setLoading(false);}};void start();return()=>{cancelled=true};},[]);
