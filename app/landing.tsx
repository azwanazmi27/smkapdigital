...
function OprDutyCentre({ notify }:{ notify:(message:string)=>void }) {
  ...
  const saveDaily=async(event:React.FormEvent)=>{ event.preventDefault(); if(!form.teachers.trim()||!form.preparedBy.trim()) return notify("Nama guru bertugas dan penyedia diperlukan"); if(Number(form.weekNumber)<1||Number(form.weekNumber)>53) return notify("Masukkan nombor minggu antara 1 hingga 53"); setSaving(true); try { const response=await fetch("/api/opr-duty",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...form,id:crypto.randomUUID(),weekNumber:Number(form.weekNumber),schoolYear:Number(form.schoolYear),dayName,teacherTotal:Number(form.teacherTotal)||0,teacherPresent:Number(form.teacherPresent)||0,teacherAbsent:Number(form.teacherAbsent)||0})}); const data=await response.json() as {error?:string}; if(!response.ok) throw new Error(data.error||"Laporan tidak dapat disimpan"); notify("Laporan harian berjaya disimpan"); setForm((current)=>({...current,activityNote:"",generalNote:""})); } catch(error){notify(error instanceof Error?error.message:"Laporan tidak dapat disimpan");} finally{setSaving(false);} };
