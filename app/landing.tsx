...
const freshDutyDetails = ():DutyDetails => ({ locationRatings:Object.fromEntries(cleanlinessLocations.map((location)=>[location,"Baik"])),disciplineControlled:true,disciplineCaseType:"",disciplineStudents:"",disciplineAction:"",hospitalMale:"0",hospitalFemale:"0",healthRoomMale:"0",healthRoomFemale:"0",hospitalTeacher:"",hospitalDriver:"",monitoringAgency:"Tiada",monitoringDetail:"",currentIssue:"" });
...
function OprDutyCentre({ notify }:{ notify:(message:string)=>void }) {
  ...
  {tab==="daily"?<form className="duty-form duty-form-complete" onSubmit={saveDaily}>
    ...
    <section className="duty-section duty-signoff"><div className="duty-section-head"><span>06</span><div><h3>Pengesahan laporan</h3><p>Pastikan semua bahagian telah disemak sebelum simpan.</p></div></div><label>Disediakan oleh<input value={form.preparedBy} onChange={(event)=>setForm((current)=>({...current,preparedBy:event.target.value}))} required/></label><div className="duty-completion"><span>✓</span><div><strong>Laporan lengkap untuk rekod pentadbiran</strong><small>Data ini akan digunakan untuk menjana rumusan mingguan secara automatik.</small></div></div></section>
