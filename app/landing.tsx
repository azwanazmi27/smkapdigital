...
function SchoolProfile(){
  ...
  return <div className="school-info-view school-profile-redesign">
    ...
    <small className="school-source-note">Maklumat asas disusun daripada sumber awam sekolah dan boleh dikemas kini oleh Admin.</small>
  </div>
}
function OrganizationChart(){
  ...
  const leaders=[...(data?.leaders||[])].sort((a,b)=>rank(a.position)-rank(b.position)||(order(a.position)<0?99:order(a.position))-(order(b.position)<0?99:order(b.position))||a.position.localeCompare(b.position));
...
function AchievementArchive({ notify }: { notify: (message: string) => void }) {
  ...
  const removeArchiveReport=async()=>{if(!deleteTarget)return;setDeleting(true);try{const token=sessionStorage.getItem("smkap_google_token")||"";const response=await fetch(`/api/drive?id=${encodeURIComponent(deleteTarget.id)}`,{method:"DELETE",headers:{Authorization:`Bearer ${token}`}}),data=await response.json() as{error?:string};if(!response.ok)throw new Error(data.error||"Laporan tidak dapat dipadam");setReports(current=>current.filter(item=>item.id!==deleteTarget.id));if(oprMemoryCache)oprMemoryCache=oprMemoryCache.filter(item=>item.id!==deleteTarget.id);setSelected(current=>current?.id===deleteTarget.id?null:current);setDeleteTarget(null);notify("Laporan berjaya dipadam daripada Google Drive");}catch(error){notify(error instanceof Error?error.message:"Laporan tidak dapat dipadam");}finally{setDeleting(false);}};
  const downloadArchiveBundle=async()=>{if(!reports.length)return notify("Tiada laporan untuk dimuat turun");setBundling(true);try{const token=sessionStorage.getItem("smkap_google_token")||"";const response=await fetch("/api/drive",{method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:JSON.stringify({action:"bundle",ids:reports.map(item=>item.id)})});if(!response.ok){const data=await response.json() as{error?:string};throw new Error(data.error||"Bundle tidak dapat disediakan");}const blob=await response.blob(),url=URL.createObjectURL(blob),link=document.createElement("a");link.href=url;link.download="Arkib-Kejayaan-SMKAP.zip";link.click();URL.revokeObjectURL(url);notify("Bundle Arkib Kejayaan berjaya dimuat turun");}catch(error){notify(error instanceof Error?error.message:"Bundle tidak dapat dimuat turun");}finally{setBundling(false);}};
  ...
  useEffect(()=>{void load(false);const token=sessionStorage.getItem("smkap_google_token")||"";if(token)void fetch("/api/admin-users?resource=me",{headers:{Authorization:`Bearer ${token}`}}).then(async response=>{const data=await response.json() as{me?:PortalIdentity};if(response.ok&&data.me){setArchiveUser(data.me);setForm(current=>({...current,uploadedBy:current.uploadedBy||data.me!.name}));}}).catch(()=>{});},[]);
  if(selected){const item=meta(selected);return <div className="achievement-shell"><div className="achievement-preview"><button onClick={()=>setSelected(null)} aria-label="Tutup pratonton">×</button><span className="modal-overline">PDF SEBENAR · GOOGLE DRIVE SEKOLAH</span><h2>{item.title}</h2><p>{new Intl.DateTimeFormat("ms-MY",{day:"numeric",month:"long",year:"numeric"}).format(new Date(`${item.date}T12:00:00`))} · {item.level}</p><div className="achievement-document"><iframe src={`/api/drive?file=${encodeURIComponent(selected.id)}`} title={`Sijil ${item.title}`}/></div><div className="achievement-preview-meta"><span><small>Pencapaian</small><strong>{item.achievement}</strong></span><span><small>Tempat</small><strong>{item.venue}</strong></span><span><small>Bidang</small><strong>{item.field}</strong></span><span><small>Dimuat naik oleh</small><strong>{item.uploadedBy}</strong></span></div><div className="achievement-preview-actions"><a href={`/api/drive?file=${encodeURIComponent(selected.id)}&download=1`}>Muat turun fail</a>{isArchiveAdmin&&<button onClick={()=>setDeleteTarget(selected)}>Padam laporan</button>}{isArchiveAdmin&&<button onClick={()=>void downloadArchiveBundle()} disabled={bundling}>{bundling?"Menyediakan ZIP…":"Muat turun semua Arkib"}</button>}</div></div>{deleteTarget&&<div className="achievement-confirm" role="alertdialog" aria-modal="true"><section><span>PADAM LAPORAN</span><h3>Padam “{meta(deleteTarget).title}”?</h3><p>Fail akan dipindahkan ke tong sampah Google Drive dan tidak lagi muncul dalam dashboard.</p><div><button onClick={()=>setDeleteTarget(null)} disabled={deleting}>Batal</button><button onClick={()=>void removeArchiveReport()} disabled={deleting}>{deleting?"Sedang memadam…":"Ya, padam laporan"}</button></div></section></div>}</div>}
...
const exportJson=async(kind:"ekeberadaan"|"ekunjung"|"etempahan")=>{setExporting(kind);try{let payload:unknown;if(kind==="ekeberadaan"){const response=await fetch("/api/ekeberadaan?resource=all",{cache:"no-store"});payload=await response.json();if(!response.ok)throw new Error("Data E-Keberadaan tidak dapat dieksport");}else if(kind==="ekunjung"){const response=await fetch("/api/ekunjung",{cache:"no-store"});payload=await response.json();if(!response.ok)throw new Error("Data E-Kunjung tidak dapat dieksport");}else{const year=new Date().getFullYear(),all:unknown[]=[];for(let month=0;month<12;month+=1){const from=`${year}-${String(month+1).padStart(2,"0")}-01`,end=new Date(year,month+1,0),to=`${year}-${String(month+1).padStart(2,"0")}-${String(end.getDate()).padStart(2,"0")}`;const response=await fetch(`/api/etempahan?from=${from}&to=${to}`,{cache:"no-store"}),data=await response.json();if(response.ok&&Array.isArray(data.bookings))all.push(...data.bookings);}payload={exportedAt:new Date().toISOString(),year,bookings:all};}saveDownload(new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),`${kind}-SMKAP-${new Date().toISOString().slice(0,10)}.json`);notify(`Data ${kind} berjaya dimuat turun`);}catch(e){notify(e instanceof Error?e.message:"Eksport tidak berjaya");}finally{setExporting("");}};
  const exportAllReports=async()=>{setExporting("reports");try{const index=await fetch("/api/drive",{cache:"no-store"}),data=await index.json() as{files?:OprReport[];error?:string};if(!index.ok||!data.files?.length)throw new Error(data.error||"Tiada laporan untuk dimuat turun");const response=await fetch("/api/drive",{method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:JSON.stringify({action:"bundle",ids:data.files.map(item=>item.id),name:`Semua-Laporan-SMKAP-${new Date().toISOString().slice(0,10)}.zip`})});if(!response.ok){const issue=await response.json() as{error?:string};throw new Error(issue.error||"Fail ZIP tidak dapat disediakan");}saveDownload(await response.blob(),`Semua-Laporan-SMKAP-${new Date().toISOString().slice(0,10)}.zip`);notify("Semua PDF OPR, laporan bertugas dan arkib berjaya dimuat turun");}catch(e){notify(e instanceof Error?e.message:"Eksport laporan tidak berjaya");}finally{setExporting("");}};
  ...
  const nav=[["dashboard","Dashboard"],["users","Pengguna"],["access","Akses Admin"],["content","Tentang Sekolah"],["announcements","Pengumuman"],["calendar","Takwim"],["modules","Modul"],["export","Eksport & Sandaran"],["trash","Tong Sampah"],["logs","Log Aktiviti"]];
