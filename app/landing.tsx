...
const portalHistoryFolders = new Set<Exclude<Folder, null>>([
  "uploads", "upload-paper", "upload-duty", "ibubapa", "warga", "assessment", "tentang", "schoolprofile", "orgchart", "announcements", "calendar", "directory", "pengunjung", "ekunjung", "ekeberadaan", "etempahan", "achievement", "epemantauan", "skas", "pengurusan", "epanitia", "oprhub", "oprgenerator", "oprduty", "admin",
]);
...
const folderContent: Record<Exclude<Folder, null | "uploads" | "upload-paper" | "upload-duty" | "admin" | "oprgenerator" | "oprduty" | "ekunjung" | "directory" | "schoolprofile" | "orgchart" | "announcements" | "calendar" | "skas" | "pengurusan" | "epanitia" | "ekeberadaan" | "etempahan" | "achievement" | "epemantauan">, { title: string; intro: string; items: SubItem[] }> = {
  ibubapa: {
...
export function LandingPortal() {
  ...
  const [taskEntry,setTaskEntry]=useState<"daily"|"weekly"|"form"|null>(null);
  ...
  const requestedModule=()=>{const value=new URLSearchParams(window.location.search).get("module");const allowed:Folder[]=["uploads","upload-paper","upload-duty","warga","oprhub","oprgenerator","oprduty","ekeberadaan","etempahan","achievement","epemantauan","skas","pengurusan","epanitia"];return allowed.includes(value as Folder)?value as Folder:null;};
  ...
  const openSubmodule=(folder:Folder,title:string)=>{setTaskEntry(null);if(folder==="ekeberadaan"){setAttendanceSplash(true);setOpen(folder);window.setTimeout(()=>setAttendanceSplash(false),1800);return;}setOpening(title);setOpen(folder);finishOpening();};
  ...
  const closeCurrentView = () => {
    ...
    if (open === "upload-paper" || open === "upload-duty") return returnTo("uploads");
  ...
  const closeViewLabel = open === "oprgenerator" || open === "oprduty"
    ...
    : open === "upload-paper" || open === "upload-duty"
      ? "Kembali ke pilihan Muat Naik"
      : open === "uploads" || open === "oprhub" || open === "assessment" || open === "etempahan" || open === "ekeberadaan" || open === "achievement" || open === "epemantauan" || (open === "skas" || open === "pengurusan" || open === "epanitia")
  ...
  return <main className="landing-shell">
    ...
    {open && <div className="folder-backdrop" onMouseDown={(e) => e.target === e.currentTarget && closeCurrentView()}>
      <section ref={folderModalRef} className={`folder-modal ${open === "oprgenerator" || open === "oprduty" || open === "oprhub" || open === "etempahan" || open === "ekeberadaan" || open === "achievement" || open === "epemantauan" || (open === "skas" || open === "pengurusan" || open === "epanitia") ? "generator-modal" : ""} ${open === "oprhub" ? "opr-hub-modal" : ""} ${open === "orgchart" ? "org-modal" : ""}`} role="dialog" aria-modal="true" aria-labelledby="folder-title">
        ...
        {identity&&open!=="admin"&&open!=="ekeberadaan"&&<div className="module-user-strip"><IdentityAvatar user={identity}/><div><small>WARGA SEKOLAH</small><strong className="module-user-name"><span className="module-user-name-text">{identity.name}</span><StaffExpertiseMarks/></strong><span>{identity.email} · {identity.position||"Warga SMKAP"}</span></div>{identity.grade&&<b>{identity.grade}</b>}</div>}
          {open === "uploads" ? <StaffWorkUploadHub openKind={kind=>setOpen(kind==='duty'?"upload-duty":"upload-paper")} /> : open === "upload-paper" || open === "upload-duty" ? <StaffWorkUpload isAdmin={!!identity&&["admin","super_admin"].includes(identity.role)} kind={open==="upload-duty"?"duty":"paper"} /> : open === "admin" ? <AdminPanel notify={notify} /> : open === "schoolprofile" ? <SchoolProfile/> : open === "orgchart" ? <OrganizationChart/> : open === "announcements" ? <PublicAnnouncements/> : open === "calendar" ? <SchoolCalendar/> : open === "directory" ? <TeacherDirectory notify={notify}/> : open === "ekunjung" ? <VisitorForm notify={notify} close={() => returnTo("pengunjung")} /> : open === "ekeberadaan" ? <ReliefIntegratedApp user={identity} /> : open === "etempahan" ? <BookingCentre notify={notify} close={() => returnTo("warga")} user={identity} initialTab={taskEntry==="form"||new URLSearchParams(window.location.search).get("tab")==="form"?"form":"dashboard"} /> : open === "achievement" ? <AchievementArchive notify={notify} /> : open === "epemantauan" ? <MonitoringCentre notify={notify} user={identity} /> : open === "pengurusan" ? <ManagementCentre notify={notify} user={identity} initialFolder={new URLSearchParams(window.location.search).get("folder")||""} openSkas={()=>setOpen("skas")} /> : open === "epanitia" ? <EPanitiaCentre notify={notify} user={identity} openOpr={(panitia)=>{setOprCategoryGroup("Kurikulum");setOprInitialCategory(oprCategoryForPanitia(panitia));setOpen("oprgenerator")}} /> : open === "skas" ? <SkasCentre notify={notify} user={identity} /> : open === "oprduty" ? <OprDutyCentre notify={notify} user={identity} initialTab={taskEntry==="daily"||taskEntry==="weekly"?taskEntry:"dashboard"} /> : open === "oprgenerator" ? <OprGenerator notify={notify} close={() => returnTo("oprhub")} user={identity} initialCategoryGroup={oprCategoryGroup} initialCategory={oprInitialCategory} /> : open === "oprhub" ? <OprDashboard create={(group) => {setOprCategoryGroup(group);setOprInitialCategory("");openSubmodule("oprgenerator","Cipta OPR baharu");}} openAssembly={() => {setOprCategoryGroup("Pengurusan");setOprInitialCategory("Pengurusan · Laporan Perhimpunan");openSubmodule("oprgenerator","Laporan Perhimpunan");}} openDuty={() => openSubmodule("oprduty","Laporan Guru Bertugas")} notify={notify} user={identity} /> : <>
...
function MonitoringCentre({ notify, user }: { notify: (message:string)=>void; user:PortalIdentity|null }) {
  ...
  const enhance=async()=>{if(!form.notes.trim())return notify("Masukkan catatan pemantauan dahulu");setEnhancing(true);try{const response=await fetch("/api/ai/monitoring-text",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:form.notes,title:form.title||"Pemantauan sekolah",category:monitoringCategory,objective:"",outcome:form.action})}),result=await response.json() as {details?:string;outcome?:string;error?:string};if(!response.ok||!result.details)throw new Error(result.error||"AI tidak dapat memproses catatan");setForm(current=>({...current,notes:result.details||current.notes,action:result.outcome||current.action}));notify("Catatan pemantauan telah dikemaskan dengan Bantuan Bang Wan");}catch(error){notify(error instanceof Error?error.message:"Perkhidmatan AI sedang mengalami gangguan sementara. Sila cuba semula.");}finally{setEnhancing(false);window.dispatchEvent(new Event("ai-usage-changed"));}};
...
function AchievementArchive({ notify }: { notify: (message: string) => void }) {
  ...
  const [readingSingle,setReadingSingle] = useState(false);
  ...
  const readSingleWithAi=async()=>{
    if(files.length!==1)return notify("Pilih satu fail sijil sahaja untuk Bantuan Bang Wan");
    const source=files[0];
    if(source.size>6_000_000)return notify("Fail mesti 6 MB atau kurang");
    setReadingSingle(true);setSingleAiMessage("");
    try{
      const pdf=source.type==="application/pdf"||source.name.toLowerCase().endsWith(".pdf")?source:await imageAsPdf(source);
      if(pdf.size>6_000_000)throw new Error("Fail PDF selepas pemprosesan melebihi 6 MB.");
      const response=await fetch("/api/achievement-ocr",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({base64:await fileBase64(pdf)})});
      const data=await response.json() as {recipient?:string;title?:string;date?:string;venue?:string;level?:string;field?:string;unitCategory?:string;achievement?:string;confidence?:number;error?:string};
      if(!response.ok)throw new Error(data.error||"Sijil tidak dapat dibaca");
      const title=[data.title,data.recipient].filter(Boolean).join(" — ");
      setForm(current=>({...current,title:title||current.title,date:data.date||current.date,venue:data.venue||current.venue,level:data.level||current.level,field:data.field||current.field,unitCategory:data.unitCategory||current.unitCategory,achievement:data.achievement||current.achievement}));
      setSingleAiMessage(data.confidence&&data.confidence>=75?"Cadangan Bang Wan telah diisi. Semak semua medan sebelum menyimpan.":"Bacaan Bang Wan mungkin tidak lengkap. Semak dan lengkapkan semua medan sebelum menyimpan.");
    }catch(error){setSingleAiMessage(error instanceof Error?error.message:"Sijil tidak dapat dibaca. Isi borang secara manual.");}
    finally{setReadingSingle(false);window.dispatchEvent(new Event("ai-usage-changed"));}
  };
  const readBundleWithAi=async()=>{
    const pending=bundleItems.filter(item=>item.blob.size<=6_000_000&&item.status!=="saved"&&(!item.recipient.trim()||!item.achievement.trim()));
    let allowance=pending.length;
    try{const response=await fetch('/api/ai/usage',{cache:'no-store'});if(response.ok){const usage=await response.json() as {remaining:number|null};if(usage.remaining!==null)allowance=Math.min(allowance,usage.remaining);}}catch{}
    if(allowance<1)return notify("Had Bantuan Bang Wan hari ini telah digunakan. Isi baki sijil secara manual atau cuba esok.");
    const readable=pending.slice(0,allowance);
    ...
    try{await Promise.all([readNext(),readNext()]);notify(allowance<pending.length?`${readable.length} halaman dibaca dengan Bantuan Bang Wan. Isi baki secara manual atau cuba esok.`:"Bacaan Bang Wan selesai. Semak semula nama dan pencapaian.");}
    finally{setReadingBundle(false);window.dispatchEvent(new Event("ai-usage-changed"));}
  };
...
const aiLabels:Record<string,string>={gemini:"Gemini",groq:"Groq",mistral:"Mistral",openrouter:"OpenRouter",cloudflare:"Cloudflare AI"};
function AIStatusList(){
  const [online,setOnline]=useState(false);
  useEffect(()=>{
    let active=true,retries=0;
    let retryTimer:ReturnType<typeof setTimeout>|undefined;
    const check=()=>{fetch("/api/ai/health",{cache:"no-store"}).then(async r=>{if(!r.ok)throw new Error("Status AI tidak tersedia");return r.json() as Promise<{providers?:Record<string,string>}>;}).then(data=>{if(active)setOnline(Object.values(data.providers||{}).some(status=>status==="online"));}).catch(()=>{if(!active)return;setOnline(false);if(retries++<2)retryTimer=setTimeout(check,2500);});};
    check();
    const interval=setInterval(check,60000);
    return()=>{active=false;clearInterval(interval);if(retryTimer)clearTimeout(retryTimer);};
  },[]);
  return <div className={`bang-wan-status ${online?"online":"offline"}`} role="status" aria-label={`Bang Wan ${online?"Online":"Offline"}`}><img className="bang-wan-robot" src="/bang-wan-opr.png" alt="" aria-hidden="true"/><div><strong>Bang Wan</strong><b><i aria-hidden="true"/>{online?"Online":"Offline"}</b></div><small>{online?"Sentiasa sedia membantu":"Cuba semula sebentar lagi"}</small></div>;
}
type AIAdminDetails={order:string[];timeoutMs:number;maxRetries:number;fallbackCount:number;providers:Record<string,{status:string;model:string;reason:string|null;quotaRemaining:string;successes:number;failures:number;averageLatencyMs:number|null}>};
function AdminAIStatus(){const [data,setData]=useState<AIAdminDetails|null>(null),[error,setError]=useState("");const load=()=>{setError("");fetch("/api/ai/health?detail=1",{cache:"no-store"}).then(async r=>{const x=await r.json();if(!r.ok)throw Error(x.error);setData(x)}).catch(e=>setError(e.message||"Status AI tidak dapat dibaca"));};useEffect(load,[]);if(error)return <p className="admin-state">{error}</p>;if(!data)return <p className="admin-state">Membaca status AI…</p>;return <section className="admin-section admin-ai"><span className="modal-overline">MULTI-AI AUTO FALLBACK</span><h3>Status dan penggunaan AI</h3><p>Urutan: {data.order.map(key=>aiLabels[key]).join(" → ")} · Timeout {Math.round(data.timeoutMs/1000)} saat · {data.maxRetries} retry maksimum.</p><div className="admin-ai-grid">{data.order.map(key=>{const item=data.providers[key];return <article key={key} className={item.status}><header><i></i><strong>{aiLabels[key]}</strong><b>{item.status==="online"?"Online":"Offline"}</b></header><dl><div><dt>Model</dt><dd>{item.model}</dd></div><div><dt>Berjaya / gagal</dt><dd>{item.successes} / {item.failures}</dd></div><div><dt>Purata latency</dt><dd>{item.averageLatencyMs===null?"Belum ada data":`${item.averageLatencyMs} ms`}</dd></div><div><dt>Kuota berbaki</dt><dd>{item.quotaRemaining}</dd></div><div><dt>Sebab / ralat terakhir</dt><dd>{item.reason||"Tiada ralat direkodkan"}</dd></div></dl></article>})}</div><p>Jumlah fallback sejak server bermula: <b>{data.fallbackCount}</b>.</p><button className="admin-save" onClick={load}>Segarkan status</button></section>}

function OprDashboard({ create, openAssembly, openDuty, notify, user }: { create: (group:string) => void; openAssembly: () => void; openDuty: () => void; notify: (message: string) => void; user: PortalIdentity | null }) {
  ...
  return <div className="opr-dashboard">
    ...
    <div className="opr-kpis">
      <article className="opr-kpi-total"><div className="opr-kpi-top"><span className="opr-kpi-icon"><FileText aria-hidden="true"/></span><span>JUMLAH OPR</span></div><strong>{oprReports.length}</strong><small>Laporan OPR yang telah dijana</small></article>
      <article className="opr-kpi-month"><div className="opr-kpi-top"><span className="opr-kpi-icon"><Users aria-hidden="true"/></span><span>BULAN INI</span></div><strong>{thisMonth}</strong><small>Laporan lengkap disimpan</small></article>
      <article className="opr-kpi-active"><div className="opr-kpi-top"><span className="opr-kpi-icon"><Presentation aria-hidden="true"/></span><span>PALING AKTIF</span></div><strong className="word">{oprReports.length ? oprCategories[mostActiveIndex][0] : "Belum ada"}</strong><small>{oprReports.length ? `${counts[mostActiveIndex]} laporan` : "Menunggu OPR pertama"}</small></article>
      <article className="ai-kpi"><span>STATUS AI</span><AIStatusList/></article>
    </div>
    <section className="opr-duty-entry"><div><span>LAPORAN GURU BERTUGAS</span><h3>Harian dan rumusan mingguan</h3><p>Semua maklumat guru bertugas di satu tempat.</p></div><CalendarDays className="opr-duty-art" aria-hidden="true"/>{signInRequired ? <a href="/signin-with-chatgpt?return_to=%2F">Log masuk untuk buka <b>→</b></a> : <button onClick={openDuty}>Buka laporan guru bertugas <b>→</b></button>}</section>
...
function AdminPanel({ notify }: { notify: (message: string) => void }) {
  ...
  const testDrive=async()=>{const controller=new AbortController(),timeout=window.setTimeout(()=>controller.abort(),15_000);setDriveState("testing");try{const response=await fetch("/api/drive?refresh=1",{cache:"no-store",signal:controller.signal}),data=await response.json();if(!response.ok)throw new Error(data.error||"Sambungan gagal");setDriveState("connected");notify(`Google Drive sekolah disahkan · ${(data.files||[]).length} fail OPR boleh dibaca`);}catch(e){setDriveState("error");notify(e instanceof DOMException&&e.name==="AbortError"?"Google Drive tidak memberi respons dalam 15 saat. Semak sambungan dan cuba Sambung semula Google Drive.":e instanceof Error?e.message:"Sambungan Google Drive gagal");}finally{window.clearTimeout(timeout)}};
...
const exportJson=async(kind:"ekeberadaan"|"etempahan")=>{setExporting(kind);try{let payload:unknown;if(kind==="ekeberadaan"){const response=await fetch("/api/ekeberadaan?resource=all",{cache:"no-store"});payload=await response.json();if(!response.ok)throw new Error("Data E-Keberadaan tidak dapat dieksport");}else{const year=new Date().getFullYear(),all:unknown[]=[];for(let month=0;month<12;month+=1){const from=`${year}-${String(month+1).padStart(2,"0")}-01`,end=new Date(year,month+1,0),to=`${year}-${String(month+1).padStart(2,"0")}-${String(end.getDate()).padStart(2,"0")}`;const response=await fetch(`/api/etempahan?from=${from}&to=${to}`,{cache:"no-store"}),data=await response.json();if(response.ok&&Array.isArray(data.bookings))all.push(...data.bookings);}payload={exportedAt:new Date().toISOString(),year,bookings:all};}saveDownload(new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),`${kind}-SMKAP-${new Date().toISOString().slice(0,10)}.json`);notify(`Data ${kind} berjaya dimuat turun`);}catch(e){notify(e instanceof Error?e.message:"Eksport tidak berjaya");}finally{setExporting("");}};
  const exportVisitorPdf=async()=>{
    setExporting("ekunjung");
    try{
      const response=await fetch(`/api/ekunjung?view=pdf&date=${encodeURIComponent(visitorExportDate)}`,{cache:"no-store"});
      if(!response.ok){let message="PDF E-Kunjung tidak dapat disediakan";try{const data=await response.json() as{error?:string};message=data.error||message;}catch{}throw new Error(message);}
      if(!response.headers.get("content-type")?.includes("application/pdf"))throw new Error("Pelayan tidak memulangkan PDF yang sah");
      saveDownload(await response.blob(),`Senarai-Pelawat-${visitorExportDate}.pdf`);
      notify(response.headers.get("X-Ekunjung-Source")==="portal-only"?"PDF rekod portal dimuat turun. Google Sheet belum dapat disemak; cuba lagi untuk salinan penuh.":"PDF senarai pelawat berjaya dimuat turun");
    }catch(error){notify(error instanceof Error?error.message:"PDF E-Kunjung tidak berjaya dijana");}
  ...
  const nav=[["dashboard","Dashboard"],["ai","Tetapan AI"],["years","Tahun Persekolahan"],["users","Pengguna"],["expertise","Kepakaran Guru"],["access","Akses Admin"],["content","Tentang Sekolah"],["announcements","Pengumuman"],["notifications","Notifikasi"],["calendar","Takwim"],["modules","Modul"],["drive","Dokumen & Google Drive"],["export","Eksport & Sandaran"],["trash","Tong Sampah"],["logs","Log Aktiviti"]];
...
function VisitorForm({ notify, close }: { notify: (message: string) => void; close: () => void }) {
  ...
  const complete = Boolean(form.date && form.timeIn && form.visitorName && form.phone && form.purpose);
  useEffect(() => {
  ...
  const saveVisit = async () => {
    ...
    try {
      const response = await fetch("/api/ekunjung", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "create", ...form, photo: photoFile ? await photoPayload(photoFile) : null }) });
  ...
  const checkoutVisit = async () => {
    setSaving(true); setSaveError("");
    try {
      const response = await fetch("/api/ekunjung", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "checkout", id: checkoutId, date: activeRecords.find(record=>record.id===checkoutId)?.date, timeOut: checkoutTime }) });
  ...
  if (review) return <article className="visitor-review">
    ...
    <div className="visitor-review-layout">{photo && <img src={photo} alt="Gambar pelawat" />}<div>{[["Nama pelawat",form.visitorName],["Nombor telefon",form.phone],["Tujuan lawatan",form.purpose],["Tarikh & masa",`${form.date} · ${form.timeIn}`],...(form.vehicleNo ? [["Nombor kenderaan",form.vehicleNo.toUpperCase()]] : []),...(form.organisation ? [["Organisasi",form.organisation]] : []),...(form.meetingPlace ? [["Tempat perjumpaan",form.meetingPlace]] : []),...(form.notes ? [["Catatan",form.notes]] : [])].map(([label,value]) => <div key={label}><small>{label}</small><strong>{value}</strong></div>)}</div></div>
    {saving && <div className="visitor-saving-state"><i></i><div><strong>Sedang menyimpan rekod...</strong><small>Jangan tutup halaman ini.</small></div></div>}
    {saveError && <p className="visitor-error">{saveError}</p>}
  ...
  return <div className="visitor-form-shell">
    ...
    <form className="visitor-form" onSubmit={(event) => { event.preventDefault(); if (complete) setReview(true); }}>
      ...
      <label>Tujuan lawatan *<select value={form.purpose} onChange={(event) => setVisitorField("purpose",event.target.value)} required><option value="">Pilih tujuan</option><option>Urusan Rasmi</option><option>Berjumpa Guru / Staf</option><option>Penghantaran Barang</option><option>Mesyuarat / Program</option><option>Urusan Murid</option><option>Kuarters</option><option>Aspura</option><option>Aspuri</option><option>Lain-lain</option></select></label>
      <label className="visitor-photo">Gambar pelawat (pilihan)<input type="file" accept="image/jpeg,image/png" capture="environment" onChange={(event) => { const file = event.target.files?.[0]; if (file) { if (photo) URL.revokeObjectURL(photo); setPhotoFile(file); setPhoto(URL.createObjectURL(file)); } }} />{photo ? <div><img src={photo} alt="Pratonton gambar pelawat" /><span>Tekan untuk tukar gambar</span></div> : <div><b>⌁</b><strong>Ambil atau pilih satu gambar</strong><span>JPG atau PNG · boleh teruskan tanpa gambar</span></div>}</label>
      {photo && <button type="button" onClick={()=>{URL.revokeObjectURL(photo);setPhoto("");setPhotoFile(null);}}>Buang gambar</button>}
      ...
      <p className="visitor-disclaimer"><span>ⓘ</span> Dengan meneruskan, pelawat bersetuju maklumat dan gambar (jika diberikan) digunakan oleh pihak sekolah bagi rekod lawatan, keselamatan dan kecemasan sahaja. Pendaftaran ini bukan kebenaran automatik untuk memasuki kawasan larangan; pelawat hendaklah mematuhi arahan pengawal dan pihak sekolah.</p>
...
function OprDutyCentre({ notify,user,initialTab="dashboard" }:{ notify:(message:string)=>void;user:PortalIdentity|null;initialTab?:"dashboard"|"daily"|"weekly" }) {
  const today=new Date().toISOString().slice(0,10), currentYear=new Date(`${today}T12:00:00`).getFullYear();
  const [tab,setTab]=useState<"dashboard"|"daily"|"weekly">(initialTab);
  ...
  const makeWeeklyPdf=async()=>{
    ...
    const headingY=146;pdf.setFillColor(45,119,165);pdf.roundedRect(12,headingY,186,9,2,2,"F");pdf.setTextColor(255,255,255);pdf.setFont("helvetica","bold");pdf.setFontSize(7.3);pdf.text("RUMUSAN MINGGUAN",16,headingY+6);pdf.setTextColor(55,75,88);pdf.setFont("helvetica","normal");pdf.setFontSize(8.2);const lines=pdf.splitTextToSize(summary,178);const firstPageCapacity=17,continuationCapacity=40;pdf.text(lines.slice(0,firstPageCapacity),16,164,{lineHeightFactor:1.35});drawPdfFooter(pdf);
    for(let offset=firstPageCapacity;offset<lines.length;offset+=continuationCapacity){pdf.addPage();drawPdfHeader(pdf,logo,"LAPORAN MINGGUAN GURU BERTUGAS",`SAMBUNGAN · MINGGU ${week}`);pdf.setTextColor(22,54,82);pdf.setFont("helvetica","bold");pdf.setFontSize(9);pdf.text("SAMBUNGAN RUMUSAN",12,51);pdf.setTextColor(55,75,88);pdf.setFont("helvetica","normal");pdf.setFontSize(8.2);pdf.text(lines.slice(offset,offset+continuationCapacity),16,61,{lineHeightFactor:1.35});drawPdfFooter(pdf);}
...
function OprGenerator({ notify, close, user, initialCategoryGroup="", initialCategory="" }: { notify: (message: string) => void; close: () => void; user:PortalIdentity|null; initialCategoryGroup?:string;initialCategory?:string }) {
  ...
  const enhance = async () => {
    ...
    try {
      ...
      if (!response.ok || !data.details) throw new Error(data.error || "AI tidak dapat memproses permintaan");
      ...
      notify("Pelaksanaan, objektif dan hasil diperkemas dengan Bantuan Bang Wan");
    } catch (error) {
      const message = error instanceof Error ? error.message : "Perkhidmatan AI sedang mengalami gangguan sementara. Sila cuba semula.";
      setAiMessage(`⚠ ${message}`);
      notify(message);
    } finally {
      setEnhancing(false);window.dispatchEvent(new Event("ai-usage-changed"));
    }
  };
  ...
  return <div className="opr-generator">
    ...
    {!preview ? <form className="generator-form" onSubmit={(event) => { event.preventDefault(); if (complete) void preparePreview(); }}>
      ...
      <fieldset className={`opr-conditional-card${competition.enabled?" is-open":""}`}><legend>Maklumat aktiviti</legend><label className="opr-form-switch"><input type="checkbox" checked={competition.enabled} onChange={(event)=>{const enabled=event.target.checked;setCompetition(current=>({...current,enabled,name:enabled?form.title:current.name}));}}/><span><strong>Aktiviti ini melibatkan pertandingan atau pencapaian</strong><small>Tandakan hanya jika terdapat penyertaan, keputusan atau anugerah.</small></span></label>{competition.enabled&&<div className="opr-conditional-fields">
      ...
      <small>Bantuan Bang Wan untuk OPR boleh digunakan tanpa had harian.</small><button type="button" className="generator-ai" onClick={enhance} disabled={enhancing}><span>✦</span><span>{enhancing ? "Bang Wan sedang menulis..." : "Jana pelaksanaan, objektif & hasil dengan Bantuan Bang Wan"}<small>Bang Wan mengekalkan fakta asal dan mengemaskan ketiga-tiga bahagian</small></span></button>
